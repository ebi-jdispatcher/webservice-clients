...
MapMi (REST) web service Perl client using L<LWP>.
...
[Optional]
  --scorethr            : int  : Score threshold. The minimum score for a candidate hairpin.
  --longext             : int  : Long match extention (bp).
  --shortext            : int  : Short match extention (bp).
  --maxmis              : int  : Maximum mature mismatches.
  --mismatchpen         : int  : Mature mismatch penalty.
  --bowtie              : int  : Maximum allowed matches per candidate genome (Bowtie-m). 0 for
                                 disable.
  --excludecan          : bool : Exclude candidates in loop. Binary toggle to exclude mature.
  --maxloop             : int  : Maximum loop overlap. Maximum allowed mature basepairs overlapping
                                 loop.
  --sequence            : str  : The query sequence(s) can be entered directly into this form. The
