...
//
// Simple Phylogeny (REST) web service Java client.
...
public class RestClientsimple_phylogeny {

    static {
        System.setProperty("org.slf4j.simpleLogger.defaultLogLevel", "ERROR");
    }

    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(RestClientsimple_phylogeny.class);
    ...
    private final String generalInfo = "EMBL-EBI Simple Phylogeny Java Client:\n"
                                   + "\n"
                                   ...
                                   + "  Usage: java -jar simple_phylogeny.jar --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: results as an attachment\n"
                                   + "\n"
                                   + "Asynchronous job:\n"
                                   + "  Use this if you want to retrieve the results at a later time. The results\n"
                                   + "  are stored for up to 24 hours.\n"
                                   + "  Usage: java -jar simple_phylogeny.jar --async --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: jobid\n"
                                   + "\n"
                                   + "Check status of Asynchronous job:\n"
                                   + "   Usage: java -jar simple_phylogeny.jar --status --jobid <jobId>\n"
                                   + "\n"
                                   + "Retrieve job data:\n"
                                   + "  Use the jobid to query for the status of the job. If the job is finished,\n"
                                   + "  it also returns the results/errors.\n"
                                   + "  Usage: java -jar simple_phylogeny.jar --polljob --jobid <jobId> [--outfile string]\n"
    ...
    public static void main(String[] args) throws IOException, InterruptedException, ServiceException, JAXBException {
        ...
        RestClientsimple_phylogeny client = new RestClientsimple_phylogeny();
        client.run(args);
    }
     ...
     */
    private RestClientsimple_phylogeny() throws IOException {

        toolId = "simple_phylogeny";
        toolName = "Simple Phylogeny";
        toolDescription = "Generating Phylogenetic Trees with Simple Phylogeny.";
    ...
    private void addGeneralOptions() {
        allOptions.addOption("tree", "", true, "Determines the outputs that the Simple Phylogeny tool produces.");
        allOptions.addOption("kimura", "", false, "Controls whether Simple Phylogeny attempts to correct for multiple"
                   ...
                   + "corrected.");
        allOptions.addOption("tossgaps", "", false, "With this option enabled columns where any of the sequences in the"
        ...
        allOptions.addOption("clustering", "", true, "Clustering Methods");
        allOptions.addOption("pim", "", false, "Output the percentage identity matrix");
        allOptions.addOption("sequence", "", true, "Phylogeny using an alignment directly entered into the input box in a"
    ...
    public WsParameters getParams() {
        ...
        if (ClientUtilssimple_phylogeny.isResponseCorrect(response, debugLevel)) {
            ...
            if (cli.hasOption("params")) {
                ...
                if (params != null) {
                    ...
                    sb.append("\n\nTo learn more about selected parameter run:\n")
                            .append("java -jar ").append("simple_phylogeny.jar")
                            .append(" --paramDetails <parameter-name>")
                            .append(params.getIds().size() > 0 ? "\n\nFor example:\njava -jar simple_phylogeny.jar --paramDetails " + params.getIds().get(0) : "");
                ...
                if (jobid != null) {
                    ...
                    if (cli.hasOption("async")) {
                        ...
                        printDebugMessage("run", String.format("You can check status of your job with:\n" +
                                                 "java -jar simple_phylogeny.jar --status --jobid %s\n", jobid), 11);

                        if (outputLevel > 0)
                            System.out.println("To check status: java -jar simple_phylogeny.jar --status --jobid " + jobid);
                    ...
                    // Synchronous execution
                        ...
                        if (ClientUtilssimple_phylogeny.getStatusInIntervals(this, jobid, pollFreq, outputLevel, debugLevel)) {
    ...
    public void downloadResults(CommandLine cli, String jobid) throws IOException {
        ...
        if (cli.hasOption("outformat")) {
            resultTypes = ClientUtilssimple_phylogeny.processUserOutputFormats(cli.getOptionValue("outformat"),
