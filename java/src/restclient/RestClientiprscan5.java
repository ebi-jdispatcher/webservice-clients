...
//
// InterProScan 5 (REST) web service Java client.
...
public class RestClientiprscan5 {
    ...
    private final String generalInfo = "EMBL-EBI InterProScan 5 Java Client:\n"
                                   + "\n"
                                   ...
                                   + "  Usage: java -jar iprscan5.jar --async --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: jobid\n"
                                   + "\n"
    ...
    public static void main(String[] args) throws IOException, InterruptedException, ServiceException, JAXBException {
        ...
        RestClientiprscan5 client = new RestClientiprscan5();
        client.run(args);
    }
     ...
     */
    private RestClientiprscan5() throws IOException {

        toolId = "iprscan5";
        toolName = "InterProScan 5";
        toolDescription = "Protein function analysis with InterProScan 5.";
    ...
    public void printGetStatus(String jobid) throws IOException, ServiceException {
        ...
        if (outputLevel > 0 && status.equals("FINISHED")) {
             System.out.println("To get results: java -jar iprscan5.jar --polljob --jobid " + jobid);
        }
    }
    ...
    private void addGeneralOptions() {
        allOptions.addOption("goterms", "", false, "Switch on look-up of corresponding Gene Ontology annotations");
        allOptions.addOption("pathways", "", false, "Switch on look-up of corresponding pathway annotations");
        allOptions.addOption("appl", "", true, "A number of different protein sequence applications are launched."
                   + "These applications search against specific databases and have"
                   + "preconfigured cut off thresholds.");
        allOptions.addOption("sequence", "", true, "Your protein sequence can be entered directly into this form in GCG,"
    ...
    public WsParameters getParams() {
        ...
        if (ClientUtilsiprscan5.isResponseCorrect(response, debugLevel)) {
        ...
        try {
            ...
            if (cli.hasOption("params")) {
                ...
                if (params != null) {
                    ...
                    sb.append("\n\nTo learn more about selected parameter run:\n")
                            .append("java -jar ").append("iprscan5.jar")
                            .append(" --paramDetails <parameter-name>")
                            .append(params.getIds().size() > 0 ? "\n\nFor example:\njava -jar iprscan5.jar --paramDetails " + params.getIds().get(0) : "");
            ...
            // Details of selected parameter
                ...
                if (parameter != null) {
                    ...
                    ClientUtilsiprscan5.marshallToXML(parameter, debugLevel, "parameters");
                } else {
            ...
            // Submit new job
                ...
                String jobid = null;
                if (sequence != null){
                    jobid = submitJob(cli, ClientUtilsiprscan5.loadData(sequence), null);
                } else if (asequence != null && bsequence != null) {
                    jobid = submitJob(cli, ClientUtilsiprscan5.loadData(asequence),
                                             ClientUtilsiprscan5.loadData(bsequence));
                }
                ...
                if (jobid != null) {
                    ...
                    if (cli.hasOption("async")) {
                        ...
                        printDebugMessage("run", String.format("You can check status of your job with:\n" +
                                                 "java -jar iprscan5.jar --status --jobid %s\n", jobid), 11);
                    ...
                    // Synchronous execution
                        ...
                        if (ClientUtilsiprscan5.getStatusInIntervals(this, jobid, pollFreq, outputLevel, debugLevel)) {
            ...
            // Check already submitted job
                ...
                if (cli.hasOption("resultTypes")) {
                    ...
                    ClientUtilsiprscan5.marshallToXML(getResultTypesForJobId(jobid), debugLevel, "types");
                    if (outputLevel > 0)
                        System.out.println("To get results:\n  java -jar iprscan5.jar --polljob --jobid " + jobid
                                           + "\n  java -jar iprscan5.jar --polljob --outformat <type> --jobid " + jobid);
                }
    ...
    public void downloadResults(CommandLine cli, String jobid) throws IOException {
        ...
        if (cli.hasOption("outformat")) {
            resultTypes = ClientUtilsiprscan5.processUserOutputFormats(cli.getOptionValue("outformat"),
    ...
    private String submitJob(CommandLine cli, String inputSeq, String inputSeq2)
        ...
        if (cli.hasOption("goterms") == false)
           form.putSingle("goterms", "true");

        if (cli.hasOption("pathways") == false)
           form.putSingle("pathways", "true");

        if (cli.hasOption("appl") == false)
           form.putSingle("appl", "MobiDBLite");
