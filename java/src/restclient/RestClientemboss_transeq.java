...
//
// EMBOSS transeq (REST) web service Java client.
...
public class RestClientemboss_transeq {

    static {
        System.setProperty("org.slf4j.simpleLogger.defaultLogLevel", "ERROR");
    }

    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(RestClientemboss_transeq.class);
    ...
    private final String generalInfo = "EMBL-EBI EMBOSS transeq Java Client:\n"
                                   + "\n"
                                   ...
                                   + "                        your input sequence before frame translatio.\n"
                                   ...
                                   + "  Usage: java -jar emboss_transeq.jar --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: results as an attachment\n"
                                   + "\n"
                                   + "Asynchronous job:\n"
                                   + "  Use this if you want to retrieve the results at a later time. The results\n"
                                   + "  are stored for up to 24 hours.\n"
                                   + "  Usage: java -jar emboss_transeq.jar --async --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: jobid\n"
                                   + "\n"
                                   + "Check status of Asynchronous job:\n"
                                   + "   Usage: java -jar emboss_transeq.jar --status --jobid <jobId>\n"
                                   + "\n"
                                   + "Retrieve job data:\n"
                                   + "  Use the jobid to query for the status of the job. If the job is finished,\n"
                                   + "  it also returns the results/errors.\n"
                                   + "  Usage: java -jar emboss_transeq.jar --polljob --jobid <jobId> [--outfile string]\n"
    ...
    public static void main(String[] args) throws IOException, InterruptedException, ServiceException, JAXBException {
        ...
        RestClientemboss_transeq client = new RestClientemboss_transeq();
        client.run(args);
    }
     ...
     */
    private RestClientemboss_transeq() throws IOException {

        toolId = "emboss_transeq";
        toolName = "EMBOSS transeq";
        toolDescription = "Sequence translations with transeq.";
    ...
    public void printGetStatus(String jobid) throws IOException, ServiceException {
        ...
        if (outputLevel > 0 && status.equals("FINISHED")) {
             System.out.println("To get results: java -jar emboss_transeq.jar --polljob --jobid " + jobid);
        }
    }
    ...
    private void addGeneralOptions() {
        allOptions.addOption("frame", "", true, "The frames to be translated. The order of the frames follows the"
        ...
        allOptions.addOption("codontable", "", true, "Which genetic code table to use. These are kept synchronised with"
                   + "those maintained at the NCBIs Taxonomy Browser.");
        allOptions.addOption("regions", "", true, "Which regions of the users DNA molecule are to be translated.");
        allOptions.addOption("trim", "", false, "Remove * and X (stop and ambiguity) symbols from the end of the"
                   + "translation.");
        allOptions.addOption("reverse", "", false, "Choose this option if you wish to reverse and complement your input"
                   + "sequence before frame translation.");
        allOptions.addOption("sequence", "", true, "Any input formats accepted by EMBOSS can be used, the full list of"
                   ...
                   + "to save files with the Unix format option to avoid hidden Windows"
                   + "characters.");
    ...
    public WsParameters getParams() {
        ...
        if (ClientUtilsemboss_transeq.isResponseCorrect(response, debugLevel)) {
        ...
        try {
            ...
            if (cli.hasOption("params")) {
                ...
                if (params != null) {
                    ...
                    sb.append("\n\nTo learn more about selected parameter run:\n")
                            .append("java -jar ").append("emboss_transeq.jar")
                            .append(" --paramDetails <parameter-name>")
                            .append(params.getIds().size() > 0 ? "\n\nFor example:\njava -jar emboss_transeq.jar --paramDetails " + params.getIds().get(0) : "");
            ...
            // Details of selected parameter
                ...
                if (parameter != null) {
                    ...
                    ClientUtilsemboss_transeq.marshallToXML(parameter, debugLevel, "parameters");
                } else {
            ...
            // Submit new job
                ...
                String jobid = null;
                if (sequence != null){
                    jobid = submitJob(cli, ClientUtilsemboss_transeq.loadData(sequence), null);
                } else if (asequence != null && bsequence != null) {
                    jobid = submitJob(cli, ClientUtilsemboss_transeq.loadData(asequence),
                                             ClientUtilsemboss_transeq.loadData(bsequence));
                }
                ...
                if (jobid != null) {
                    ...
                    if (cli.hasOption("async")) {
                        ...
                        printDebugMessage("run", String.format("You can check status of your job with:\n" +
                                                 "java -jar emboss_transeq.jar --status --jobid %s\n", jobid), 11);

                        if (outputLevel > 0)
                            System.out.println("To check status: java -jar emboss_transeq.jar --status --jobid " + jobid);
                    ...
                    // Synchronous execution
                        ...
                        if (ClientUtilsemboss_transeq.getStatusInIntervals(this, jobid, pollFreq, outputLevel, debugLevel)) {
            ...
            // Check already submitted job
                ...
                if (cli.hasOption("resultTypes")) {
                    ...
                    ClientUtilsemboss_transeq.marshallToXML(getResultTypesForJobId(jobid), debugLevel, "types");
                    if (outputLevel > 0)
                        System.out.println("To get results:\n  java -jar emboss_transeq.jar --polljob --jobid " + jobid
                                           + "\n  java -jar emboss_transeq.jar --polljob --outformat <type> --jobid " + jobid);
                }
    ...
    public void downloadResults(CommandLine cli, String jobid) throws IOException {
        ...
        if (cli.hasOption("outformat")) {
            resultTypes = ClientUtilsemboss_transeq.processUserOutputFormats(cli.getOptionValue("outformat"),
    ...
    private String submitJob(CommandLine cli, String inputSeq, String inputSeq2)
        ...
        if (cli.hasOption("frame") == false)
           form.putSingle("frame", "1");

        if (cli.hasOption("codontable") == false)
           form.putSingle("codontable", "0");

        if (cli.hasOption("trim") == false)
           form.putSingle("trim", "false");
