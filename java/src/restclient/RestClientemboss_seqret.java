...
//
// EMBOSS seqret (REST) web service Java client.
...
public class RestClientemboss_seqret {

    static {
        System.setProperty("org.slf4j.simpleLogger.defaultLogLevel", "ERROR");
    }

    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(RestClientemboss_seqret.class);
    ...
    private final String generalInfo = "EMBL-EBI EMBOSS seqret Java Client:\n"
                                   + "\n"
                                   ...
                                   + "  --feature             Use feature informatio.\n"
                                   ...
                                   + "  Usage: java -jar emboss_seqret.jar --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: results as an attachment\n"
                                   + "\n"
                                   + "Asynchronous job:\n"
                                   + "  Use this if you want to retrieve the results at a later time. The results\n"
                                   + "  are stored for up to 24 hours.\n"
                                   + "  Usage: java -jar emboss_seqret.jar --async --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: jobid\n"
                                   + "\n"
                                   + "Check status of Asynchronous job:\n"
                                   + "   Usage: java -jar emboss_seqret.jar --status --jobid <jobId>\n"
                                   + "\n"
                                   + "Retrieve job data:\n"
                                   + "  Use the jobid to query for the status of the job. If the job is finished,\n"
                                   + "  it also returns the results/errors.\n"
                                   + "  Usage: java -jar emboss_seqret.jar --polljob --jobid <jobId> [--outfile string]\n"
    ...
    public static void main(String[] args) throws IOException, InterruptedException, ServiceException, JAXBException {
        ...
        RestClientemboss_seqret client = new RestClientemboss_seqret();
        client.run(args);
    }
     ...
     */
    private RestClientemboss_seqret() throws IOException {

        toolId = "emboss_seqret";
        toolName = "EMBOSS seqret";
        toolDescription = "Sequenc format conversion with seqret.";
    ...
    public void printGetStatus(String jobid) throws IOException, ServiceException {
        ...
        if (outputLevel > 0 && status.equals("FINISHED")) {
             System.out.println("To get results: java -jar emboss_seqret.jar --polljob --jobid " + jobid);
        }
    }
    ...
    private void addGeneralOptions() {
        allOptions.addOption("stype", "", true, "Indicates if the query sequence is protein, DNA or RNA.");
        allOptions.addOption("inputformat", "", true, "Input format name");
        allOptions.addOption("outputformat", "", true, "Output format name.");
        allOptions.addOption("feature", "", false, "Use feature information");
        allOptions.addOption("firstonly", "", false, "Read one sequence and stop");
        allOptions.addOption("reverse", "", false, "Reverse-complement of input DNA sequences");
        allOptions.addOption("outputcase", "", true, "Change alphabet case for output sequences.");
        ...
        allOptions.addOption("sequence", "", true, "One or more sequences to be translated can be entered directly into"
    ...
    public WsParameters getParams() {
        ...
        if (ClientUtilsemboss_seqret.isResponseCorrect(response, debugLevel)) {
        ...
        try {
            ...
            if (cli.hasOption("params")) {
                ...
                if (params != null) {
                    ...
                    sb.append("\n\nTo learn more about selected parameter run:\n")
                            .append("java -jar ").append("emboss_seqret.jar")
                            .append(" --paramDetails <parameter-name>")
                            .append(params.getIds().size() > 0 ? "\n\nFor example:\njava -jar emboss_seqret.jar --paramDetails " + params.getIds().get(0) : "");
            ...
            // Details of selected parameter
                ...
                if (parameter != null) {
                    ...
                    ClientUtilsemboss_seqret.marshallToXML(parameter, debugLevel, "parameters");
                } else {
            ...
            // Submit new job
                ...
                String jobid = null;
                if (sequence != null){
                    jobid = submitJob(cli, ClientUtilsemboss_seqret.loadData(sequence), null);
                } else if (asequence != null && bsequence != null) {
                    jobid = submitJob(cli, ClientUtilsemboss_seqret.loadData(asequence),
                                             ClientUtilsemboss_seqret.loadData(bsequence));
                }
                ...
                if (jobid != null) {
                    ...
                    if (cli.hasOption("async")) {
                        ...
                        printDebugMessage("run", String.format("You can check status of your job with:\n" +
                                                 "java -jar emboss_seqret.jar --status --jobid %s\n", jobid), 11);

                        if (outputLevel > 0)
                            System.out.println("To check status: java -jar emboss_seqret.jar --status --jobid " + jobid);
                    ...
                    // Synchronous execution
                        ...
                        if (ClientUtilsemboss_seqret.getStatusInIntervals(this, jobid, pollFreq, outputLevel, debugLevel)) {
            ...
            // Check already submitted job
                ...
                if (cli.hasOption("resultTypes")) {
                    ...
                    ClientUtilsemboss_seqret.marshallToXML(getResultTypesForJobId(jobid), debugLevel, "types");
                    if (outputLevel > 0)
                        System.out.println("To get results:\n  java -jar emboss_seqret.jar --polljob --jobid " + jobid
                                           + "\n  java -jar emboss_seqret.jar --polljob --outformat <type> --jobid " + jobid);
                }
    ...
    public void downloadResults(CommandLine cli, String jobid) throws IOException {
        ...
        if (cli.hasOption("outformat")) {
            resultTypes = ClientUtilsemboss_seqret.processUserOutputFormats(cli.getOptionValue("outformat"),
    ...
    private String submitJob(CommandLine cli, String inputSeq, String inputSeq2)
        ...
        if (cli.hasOption("inputformat") == false)
           form.putSingle("inputformat", "unknown");

        if (cli.hasOption("outputformat") == false)
           form.putSingle("outputformat", "embl");

        if (cli.hasOption("feature") == false)
           form.putSingle("feature", "true");

        if (cli.hasOption("firstonly") == false)
           form.putSingle("firstonly", "false");

        if (cli.hasOption("reverse") == false)
           form.putSingle("reverse", "false");

        if (cli.hasOption("outputcase") == false)
           form.putSingle("outputcase", "none");
