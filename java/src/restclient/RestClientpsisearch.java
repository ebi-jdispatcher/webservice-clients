...
//
// PSI-Search (REST) web service Java client.
...
public class RestClientpsisearch {

    static {
        System.setProperty("org.slf4j.simpleLogger.defaultLogLevel", "ERROR");
    }

    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(RestClientpsisearch.class);
    ...
    private final String generalInfo = "EMBL-EBI PSI-Search Java Client:\n"
                                   + "\n"
                                   ...
                                   + "  --previousjobid       The job identifier for the previous PSI-Search iteratio.\n"
                                   ...
                                   + "  Usage: java -jar psisearch.jar --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: results as an attachment\n"
                                   + "\n"
                                   + "Asynchronous job:\n"
                                   + "  Use this if you want to retrieve the results at a later time. The results\n"
                                   + "  are stored for up to 24 hours.\n"
                                   + "  Usage: java -jar psisearch.jar --async --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: jobid\n"
                                   + "\n"
                                   + "Check status of Asynchronous job:\n"
                                   + "   Usage: java -jar psisearch.jar --status --jobid <jobId>\n"
                                   + "\n"
                                   + "Retrieve job data:\n"
                                   + "  Use the jobid to query for the status of the job. If the job is finished,\n"
                                   + "  it also returns the results/errors.\n"
                                   + "  Usage: java -jar psisearch.jar --polljob --jobid <jobId> [--outfile string]\n"
    ...
    public static void main(String[] args) throws IOException, InterruptedException, ServiceException, JAXBException {
        ...
        RestClientpsisearch client = new RestClientpsisearch();
        client.run(args);
    }
     ...
     */
    private RestClientpsisearch() throws IOException {

        toolId = "psisearch";
        toolName = "PSI-Search";
        toolDescription = "Sequence similarity search with PSI-Search.";
    ...
    public void printGetStatus(String jobid) throws IOException, ServiceException {
        ...
        if (outputLevel > 0 && status.equals("FINISHED")) {
             System.out.println("To get results: java -jar psisearch.jar --polljob --jobid " + jobid);
        }
    }
    ...
    private void addGeneralOptions() {
        ...
        allOptions.addOption("mask", "", false, "Turn on/off the sequence masking for HOEs in PSSM constructions. This"
        ...
        allOptions.addOption("scores", "", true, "Maximum number of alignment score summaries reported in the result"
                   + "output.");
        allOptions.addOption("alignments", "", true, "Maximum number of alignments reported in the result output.");
        allOptions.addOption("hsps", "", false, "Turn on/off the display of all significant alignments between query"
                   + "and database sequence.");
        allOptions.addOption("scoreformat", "", true, "Different score formats.");
        allOptions.addOption("filter", "", true, "Filter regions of low sequence complexity. This can avoid issues with"
                   ...
                   + "caution.");
        allOptions.addOption("hist", "", false, "Turn on/off the histogram in the PSI-Search result. The histogram"
                   + "gives a qualitative view of how well the statistical theory fits the"
                   + "similarity scores calculated by the program.");
        allOptions.addOption("annotfeats", "", false, "Turn on/off annotation features. Annotation features shows features"
        ...
        allOptions.addOption("previousjobid", "", true, "The job identifier for the previous PSI-Search iteration.");
        allOptions.addOption("selectedHits", "", true, "List of identifiers from the hits of the previous iteration to use to"
                   + "construct the search PSSM for this iteration.");
        allOptions.addOption("bdrfile", "", true, "Boundary file containing boundary information for pre-selected"
    ...
    public WsParameters getParams() {
        ...
        if (ClientUtilspsisearch.isResponseCorrect(response, debugLevel)) {
        ...
        try {
            ...
            if (cli.hasOption("params")) {
                ...
                if (params != null) {
                    ...
                    sb.append("\n\nTo learn more about selected parameter run:\n")
                            .append("java -jar ").append("psisearch.jar")
                            .append(" --paramDetails <parameter-name>")
                            .append(params.getIds().size() > 0 ? "\n\nFor example:\njava -jar psisearch.jar --paramDetails " + params.getIds().get(0) : "");
            ...
            // Details of selected parameter
                ...
                if (parameter != null) {
                    ...
                    ClientUtilspsisearch.marshallToXML(parameter, debugLevel, "parameters");
                } else {
            ...
            // Submit new job
                ...
                String jobid = null;
                if (sequence != null){
                    jobid = submitJob(cli, ClientUtilspsisearch.loadData(sequence), null);
                } else if (asequence != null && bsequence != null) {
                    jobid = submitJob(cli, ClientUtilspsisearch.loadData(asequence),
                                             ClientUtilspsisearch.loadData(bsequence));
                }
                ...
                if (jobid != null) {
                    ...
                    if (cli.hasOption("async")) {
                        ...
                        printDebugMessage("run", String.format("You can check status of your job with:\n" +
                                                 "java -jar psisearch.jar --status --jobid %s\n", jobid), 11);

                        if (outputLevel > 0)
                            System.out.println("To check status: java -jar psisearch.jar --status --jobid " + jobid);
                    ...
                    // Synchronous execution
                        ...
                        if (ClientUtilspsisearch.getStatusInIntervals(this, jobid, pollFreq, outputLevel, debugLevel)) {
            ...
            // Check already submitted job
                ...
                if (cli.hasOption("resultTypes")) {
                    ...
                    ClientUtilspsisearch.marshallToXML(getResultTypesForJobId(jobid), debugLevel, "types");
                    if (outputLevel > 0)
                        System.out.println("To get results:\n  java -jar psisearch.jar --polljob --jobid " + jobid
                                           + "\n  java -jar psisearch.jar --polljob --outformat <type> --jobid " + jobid);
                }
    ...
    public void downloadResults(CommandLine cli, String jobid) throws IOException {
        ...
        if (cli.hasOption("outformat")) {
            resultTypes = ClientUtilspsisearch.processUserOutputFormats(cli.getOptionValue("outformat"),
    ...
    private String submitJob(CommandLine cli, String inputSeq, String inputSeq2)
        ...
        if (cli.hasOption("mask") == false)
           form.putSingle("mask", "true");
        ...
        if (cli.hasOption("scoreformat") == false)
           form.putSingle("scoreformat", "default");
