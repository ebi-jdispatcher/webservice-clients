...
//
// Prank (REST) web service Java client.
...
public class RestClientprank {

    static {
        System.setProperty("org.slf4j.simpleLogger.defaultLogLevel", "ERROR");
    }

    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(RestClientprank.class);
    ...
    private final String generalInfo = "EMBL-EBI Prank Java Client:\n"
                                   + "\n"
                                   ...
                                   + "  --noise_level         Noise level; progress and debugging informatio.\n"

                                   + "  --stay_quiet          Stay quiet; disable all progress informatio.\n"
                                   ...
                                   + "  Usage: java -jar prank.jar --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: results as an attachment\n"
                                   + "\n"
                                   + "Asynchronous job:\n"
                                   + "  Use this if you want to retrieve the results at a later time. The results\n"
                                   + "  are stored for up to 24 hours.\n"
                                   + "  Usage: java -jar prank.jar --async --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: jobid\n"
                                   + "\n"
                                   + "Check status of Asynchronous job:\n"
                                   + "   Usage: java -jar prank.jar --status --jobid <jobId>\n"
                                   + "\n"
                                   + "Retrieve job data:\n"
                                   + "  Use the jobid to query for the status of the job. If the job is finished,\n"
                                   + "  it also returns the results/errors.\n"
                                   + "  Usage: java -jar prank.jar --polljob --jobid <jobId> [--outfile string]\n"
    ...
    public static void main(String[] args) throws IOException, InterruptedException, ServiceException, JAXBException {
        ...
        RestClientprank client = new RestClientprank();
        client.run(args);
    }
     ...
     */
    private RestClientprank() throws IOException {

        toolId = "prank";
        toolName = "Prank";
        toolDescription = "Multiple sequence alignment with Prank.";
    ...
    private void addGeneralOptions() {
        ...
        allOptions.addOption("data_file", "", true, "A file containing valid sequences in FASTA format can be used as input"
        ...
        allOptions.addOption("tree_file", "", true, "Tree file in Newick Binary Format.");
        allOptions.addOption("do_njtree", "", false, "compute guide tree from input alignment");
        allOptions.addOption("do_clustalw_tree", "", false, "compute guide tree using Clustalw2");
        allOptions.addOption("model_file", "", true, "Structure Model File.");
        allOptions.addOption("output_format", "", true, "Format for output alignment file");
        allOptions.addOption("trust_insertions", "", false, "Trust inferred insertions and do not allow their later matching");
        allOptions.addOption("show_insertions_with_dots", "", false, "Show gaps created by insertions as dots, deletions as dashes");
        allOptions.addOption("use_log_space", "", false, "Use log space for probabilities; slower but necessary for large"
                   + "numbers of sequences");
        allOptions.addOption("use_codon_model", "", false, "Use codon substutition model for alignment; requires DNA, multiples of"
                   + "three in length");
        allOptions.addOption("translate_DNA", "", false, "Translate DNA sequences to proteins and backtranslate results");
        allOptions.addOption("mt_translate_DNA", "", false, "Translate DNA sequences to mt proteins, align and backtranslate"
                   + "results");
        allOptions.addOption("gap_rate", "", true, "Gap Opening Rate");
        allOptions.addOption("gap_extension", "", true, "Gap Extension Probability");
        allOptions.addOption("tn93_kappa", "", true, "Parameter kappa for Tamura-Nei DNA substitution model");
        allOptions.addOption("tn93_rho", "", true, "Parameter rho for Tamura-Nei DNA substitution model");
        allOptions.addOption("guide_pairwise_distance", "", true, "Fixed pairwise distance used for generating scoring matrix in guide"
                   + "tree computation");
        allOptions.addOption("max_pairwise_distance", "", true, "Maximum pairwise distance allowed in progressive steps of multiple"
                   + "alignment; allows making matching more stringent or flexible");
        allOptions.addOption("branch_length_scaling", "", true, "Factor for scaling all branch lengths");
        allOptions.addOption("branch_length_fixed", "", true, "Fixed value for all branch lengths");
        allOptions.addOption("branch_length_maximum", "", true, "Upper limit for branch lengths");
        allOptions.addOption("use_real_branch_lengths", "", false, "Use real branch lengths; using this can be harmful as scoring matrices"
                   + "became flat for large distances; rather use max_pairwise_distance");
        allOptions.addOption("do_no_posterior", "", false, "Do not compute posterior probability; much faster if those not needed");
        allOptions.addOption("run_once", "", false, "Do not iterate alignment");
        allOptions.addOption("run_twice", "", false, "Iterate alignment");
        allOptions.addOption("penalise_terminal_gaps", "", false, "Penalise terminal gaps as any other gap");
        allOptions.addOption("do_posterior_only", "", false, "Compute posterior probabilities for given *aligned* sequences; may be"
                   + "unstable but useful");
        allOptions.addOption("use_chaos_anchors", "", false, "Use chaos anchors to massively speed up alignments; DNA only");
        allOptions.addOption("minimum_anchor_distance", "", true, "Minimum chaos anchor distance");
        allOptions.addOption("maximum_anchor_distance", "", true, "Maximum chaos anchor distance");
        allOptions.addOption("skip_anchor_distance", "", true, "Chaos anchor skip distance");
        allOptions.addOption("drop_anchor_distance", "", true, "Chaos anchor drop distance");
        allOptions.addOption("output_ancestors", "", false, "Output ancestral sequences and probability profiles; note additional"
                   + "files");
        allOptions.addOption("noise_level", "", true, "Noise level; progress and debugging information");
        allOptions.addOption("stay_quiet", "", false, "Stay quiet; disable all progress information");
        allOptions.addOption("random_seed", "", true, "Set seed for random number generator; not recommended");
    ...
    public WsParameters getParams() {
        ...
        if (ClientUtilsprank.isResponseCorrect(response, debugLevel)) {
            ...
            if (cli.hasOption("params")) {
                ...
                if (params != null) {
                    ...
                    sb.append("\n\nTo learn more about selected parameter run:\n")
                            .append("java -jar ").append("prank.jar")
                            .append(" --paramDetails <parameter-name>")
                            .append(params.getIds().size() > 0 ? "\n\nFor example:\njava -jar prank.jar --paramDetails " + params.getIds().get(0) : "");
                ...
                if (jobid != null) {
                    ...
                    if (cli.hasOption("async")) {
                        ...
                        printDebugMessage("run", String.format("You can check status of your job with:\n" +
                                                 "java -jar prank.jar --status --jobid %s\n", jobid), 11);

                        if (outputLevel > 0)
                            System.out.println("To check status: java -jar prank.jar --status --jobid " + jobid);
                    ...
                    // Synchronous execution
                        ...
                        if (ClientUtilsprank.getStatusInIntervals(this, jobid, pollFreq, outputLevel, debugLevel)) {
    ...
    public void downloadResults(CommandLine cli, String jobid) throws IOException {
        ...
        if (cli.hasOption("outformat")) {
            resultTypes = ClientUtilsprank.processUserOutputFormats(cli.getOptionValue("outformat"),
