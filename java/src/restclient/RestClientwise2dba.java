...
//
// Wise2dba (REST) web service Java client.
...
public class RestClientwise2dba {

    static {
        System.setProperty("org.slf4j.simpleLogger.defaultLogLevel", "ERROR");
    }

    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(RestClientwise2dba.class);
    ...
    private final String generalInfo = "EMBL-EBI Wise2dba Java Client:\n"
                                   + "\n"
                                   ...
                                   + "                        are not accepted. Adding a return to the end of the sequence\n"
                                   ...
                                   + "  --bsequence           The second DNA sequence to be aligned can be entered\n"
                                   + "                        directly into the form. The sequence must be in a recognised\n"
                                   + "                        format eg. GCG, FASTA, EMBL, GenBank. Partially formatted\n"
                                   + "                        sequences are not accepted. Adding a return to the end of\n"
                                   + "                        the sequence may help certain applications understand the\n"
                                   + "                        input. Note that directly using data from word processors\n"
                                   + "                        may yield unpredictable results as hidden/control characters\n"
                                   ...
                                   + "  Usage: java -jar wise2dba.jar --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: results as an attachment\n"
                                   + "\n"
                                   + "Asynchronous job:\n"
                                   + "  Use this if you want to retrieve the results at a later time. The results\n"
                                   + "  are stored for up to 24 hours.\n"
                                   + "  Usage: java -jar wise2dba.jar --async --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: jobid\n"
                                   + "\n"
                                   + "Check status of Asynchronous job:\n"
                                   + "   Usage: java -jar wise2dba.jar --status --jobid <jobId>\n"
                                   + "\n"
                                   + "Retrieve job data:\n"
                                   + "  Use the jobid to query for the status of the job. If the job is finished,\n"
                                   + "  it also returns the results/errors.\n"
                                   + "  Usage: java -jar wise2dba.jar --polljob --jobid <jobId> [--outfile string]\n"
    ...
    public static void main(String[] args) throws IOException, InterruptedException, ServiceException, JAXBException {
        ...
        RestClientwise2dba client = new RestClientwise2dba();
        client.run(args);
    }
     ...
     */
    private RestClientwise2dba() throws IOException {

        toolId = "wise2dba";
        toolName = "Wise2dba";
        toolDescription = "Pairwise sequence alignment with Wise2dba.";
    ...
    public void printGetStatus(String jobid) throws IOException, ServiceException {
        ...
        if (outputLevel > 0 && status.equals("FINISHED")) {
             System.out.println("To get results: java -jar wise2dba.jar --polljob --jobid " + jobid);
        }
    }
    ...
    private void addGeneralOptions() {
        allOptions.addOption("para", "", false, "Show parameters in output alignmment, as in genewise.");
        allOptions.addOption("pretty", "", false, "Show pretty ASCII alignment viewing, as in genewise.");
        allOptions.addOption("asequence", "", true, "The first DNA sequence to be aligned can be entered directly into the"
                   ...
                   + "entry.");
        allOptions.addOption("bsequence", "", true, "The second DNA sequence to be aligned can be entered directly into the"
                   + "form. The sequence must be in a recognised format eg. GCG, FASTA,"
                   + "EMBL, GenBank. Partially formatted sequences are not accepted. Adding"
                   + "a return to the end of the sequence may help certain applications"
                   + "understand the input. Note that directly using data from word"
    ...
    public WsParameters getParams() {
        ...
        if (ClientUtilswise2dba.isResponseCorrect(response, debugLevel)) {
        ...
        try {
            ...
            if (cli.hasOption("params")) {
                ...
                if (params != null) {
                    ...
                    sb.append("\n\nTo learn more about selected parameter run:\n")
                            .append("java -jar ").append("wise2dba.jar")
                            .append(" --paramDetails <parameter-name>")
                            .append(params.getIds().size() > 0 ? "\n\nFor example:\njava -jar wise2dba.jar --paramDetails " + params.getIds().get(0) : "");
            ...
            // Details of selected parameter
                ...
                if (parameter != null) {
                    ...
                    ClientUtilswise2dba.marshallToXML(parameter, debugLevel, "parameters");
                } else {
            ...
            // Submit new job
                ...
                String jobid = null;
                if (sequence != null){
                    jobid = submitJob(cli, ClientUtilswise2dba.loadData(sequence), null);
                } else if (asequence != null && bsequence != null) {
                    jobid = submitJob(cli, ClientUtilswise2dba.loadData(asequence),
                                             ClientUtilswise2dba.loadData(bsequence));
                }
                ...
                if (jobid != null) {
                    ...
                    if (cli.hasOption("async")) {
                        ...
                        printDebugMessage("run", String.format("You can check status of your job with:\n" +
                                                 "java -jar wise2dba.jar --status --jobid %s\n", jobid), 11);

                        if (outputLevel > 0)
                            System.out.println("To check status: java -jar wise2dba.jar --status --jobid " + jobid);
                    ...
                    // Synchronous execution
                        ...
                        if (ClientUtilswise2dba.getStatusInIntervals(this, jobid, pollFreq, outputLevel, debugLevel)) {
            ...
            // Check already submitted job
                ...
                if (cli.hasOption("resultTypes")) {
                    ...
                    ClientUtilswise2dba.marshallToXML(getResultTypesForJobId(jobid), debugLevel, "types");
                    if (outputLevel > 0)
                        System.out.println("To get results:\n  java -jar wise2dba.jar --polljob --jobid " + jobid
                                           + "\n  java -jar wise2dba.jar --polljob --outformat <type> --jobid " + jobid);
                }
    ...
    public void downloadResults(CommandLine cli, String jobid) throws IOException {
        ...
        if (cli.hasOption("outformat")) {
            resultTypes = ClientUtilswise2dba.processUserOutputFormats(cli.getOptionValue("outformat"),
