...
//
// HMMER 3 hmmscan (REST) web service Java client.
...
public class RestClienthmmer3_hmmscan {

    static {
        System.setProperty("org.slf4j.simpleLogger.defaultLogLevel", "ERROR");
    }

    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(RestClienthmmer3_hmmscan.class);
    ...
    private final String generalInfo = "EMBL-EBI HMMER 3 hmmscan Java Client:\n"
                                   + "\n"
                                   ...
                                   + "  Usage: java -jar hmmer3_hmmscan.jar --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: results as an attachment\n"
                                   + "\n"
                                   + "Asynchronous job:\n"
                                   + "  Use this if you want to retrieve the results at a later time. The results\n"
                                   + "  are stored for up to 24 hours.\n"
                                   + "  Usage: java -jar hmmer3_hmmscan.jar --async --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: jobid\n"
                                   + "\n"
                                   + "Check status of Asynchronous job:\n"
                                   + "   Usage: java -jar hmmer3_hmmscan.jar --status --jobid <jobId>\n"
                                   + "\n"
                                   + "Retrieve job data:\n"
                                   + "  Use the jobid to query for the status of the job. If the job is finished,\n"
                                   + "  it also returns the results/errors.\n"
                                   + "  Usage: java -jar hmmer3_hmmscan.jar --polljob --jobid <jobId> [--outfile string]\n"
    ...
    public static void main(String[] args) throws IOException, InterruptedException, ServiceException, JAXBException {
        ...
        RestClienthmmer3_hmmscan client = new RestClienthmmer3_hmmscan();
        client.run(args);
    }
     ...
     */
    private RestClienthmmer3_hmmscan() throws IOException {

        toolId = "hmmer3_hmmscan";
        toolName = "HMMER 3 hmmscan";
        toolDescription = "Protein function analysis with HMMER 3 hmmscan.";
    ...
    private void addGeneralOptions() {
        allOptions.addOption("incE", "", true, "Significance E-values[Model]");
        allOptions.addOption("incdomE", "", true, "Significance E-values[Hit]");
        allOptions.addOption("E", "", true, "Report E-values[Model]");
        ...
        allOptions.addOption("cut_ga", "", false, "Use the gathering threshold.");
        allOptions.addOption("nobias", "", false, "Filters");
        allOptions.addOption("hmmdbparam", "", true, "hmmdbparam");
        allOptions.addOption("alignView", "", false, "Output alignment in result");
        allOptions.addOption("database", "", true, "HMM Database");
    ...
    public WsParameters getParams() {
        ...
        if (ClientUtilshmmer3_hmmscan.isResponseCorrect(response, debugLevel)) {
            ...
            if (cli.hasOption("params")) {
                ...
                if (params != null) {
                    ...
                    sb.append("\n\nTo learn more about selected parameter run:\n")
                            .append("java -jar ").append("hmmer3_hmmscan.jar")
                            .append(" --paramDetails <parameter-name>")
                            .append(params.getIds().size() > 0 ? "\n\nFor example:\njava -jar hmmer3_hmmscan.jar --paramDetails " + params.getIds().get(0) : "");
                ...
                if (jobid != null) {
                    ...
                    if (cli.hasOption("async")) {
                        ...
                        printDebugMessage("run", String.format("You can check status of your job with:\n" +
                                                 "java -jar hmmer3_hmmscan.jar --status --jobid %s\n", jobid), 11);

                        if (outputLevel > 0)
                            System.out.println("To check status: java -jar hmmer3_hmmscan.jar --status --jobid " + jobid);
                    ...
                    // Synchronous execution
                        ...
                        if (ClientUtilshmmer3_hmmscan.getStatusInIntervals(this, jobid, pollFreq, outputLevel, debugLevel)) {
    ...
    public void downloadResults(CommandLine cli, String jobid) throws IOException {
        ...
        if (cli.hasOption("outformat")) {
            resultTypes = ClientUtilshmmer3_hmmscan.processUserOutputFormats(cli.getOptionValue("outformat"),
