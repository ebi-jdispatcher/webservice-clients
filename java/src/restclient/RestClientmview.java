...
//
// Mview (REST) web service Java client.
...
public class RestClientmview {

    static {
        System.setProperty("org.slf4j.simpleLogger.defaultLogLevel", "ERROR");
    }

    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(RestClientmview.class);
    ...
    private final String generalInfo = "EMBL-EBI Mview Java Client:\n"
                                   + "\n"
                                   ...
                                   + "  Usage: java -jar mview.jar --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: results as an attachment\n"
                                   + "\n"
                                   + "Asynchronous job:\n"
                                   + "  Use this if you want to retrieve the results at a later time. The results\n"
                                   + "  are stored for up to 24 hours.\n"
                                   + "  Usage: java -jar mview.jar --async --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: jobid\n"
                                   + "\n"
                                   + "Check status of Asynchronous job:\n"
                                   + "   Usage: java -jar mview.jar --status --jobid <jobId>\n"
                                   + "\n"
                                   + "Retrieve job data:\n"
                                   + "  Use the jobid to query for the status of the job. If the job is finished,\n"
                                   + "  it also returns the results/errors.\n"
                                   + "  Usage: java -jar mview.jar --polljob --jobid <jobId> [--outfile string]\n"
    ...
    public static void main(String[] args) throws IOException, InterruptedException, ServiceException, JAXBException {
        ...
        RestClientmview client = new RestClientmview();
        client.run(args);
    }
     ...
     */
    private RestClientmview() throws IOException {

        toolId = "mview";
        toolName = "Mview";
        toolDescription = "Multiple sequence alignment viewing with MView.";
    ...
    private void addGeneralOptions() {
        allOptions.addOption("stype", "", true, "Indicates if the sequences to align are protein or nucleotide"
                   + "(DNA/RNA).");
        allOptions.addOption("informat", "", true, "Format of the input sequence similarity search result or multiple"
                   + "sequence alignment to be processed.");
        allOptions.addOption("outputformat", "", true, "Output format for the alignment.");
        allOptions.addOption("htmlmarkup", "", true, "Amount of HTML markup to be used in the result.");
        allOptions.addOption("css", "", false, "Use Cascading Style Sheets");
        allOptions.addOption("pcid", "", true, "Compute percent identities with respect to");
        allOptions.addOption("alignment", "", false, "Show or hide the aligned sequences.");
        allOptions.addOption("ruler", "", false, "Show or hide the ruler showing the sequence coordinates.");
        allOptions.addOption("width", "", true, "Width of output alignment.");
        allOptions.addOption("coloring", "", true, "Basic style of coloring");
        allOptions.addOption("colormap", "", true, "Color map");
        allOptions.addOption("groupmap", "", true, "Group map");
        allOptions.addOption("consensus", "", false, "Show or hide consensus sequence derived from the alignment.");
        allOptions.addOption("concoloring", "", true, "Basic style of consensus coloring");
        allOptions.addOption("concolormap", "", true, "Consensus color map");
        allOptions.addOption("congroupmap", "", true, "Consensus group map");
        allOptions.addOption("congaps", "", false, "Count gaps during consensus compuatations");
        allOptions.addOption("sequence", "", true, "Sequence similarity search result (e.g. BLAST or FASTA search report)"
    ...
    public WsParameters getParams() {
        ...
        if (ClientUtilsmview.isResponseCorrect(response, debugLevel)) {
            ...
            if (cli.hasOption("params")) {
                ...
                if (params != null) {
                    ...
                    sb.append("\n\nTo learn more about selected parameter run:\n")
                            .append("java -jar ").append("mview.jar")
                            .append(" --paramDetails <parameter-name>")
                            .append(params.getIds().size() > 0 ? "\n\nFor example:\njava -jar mview.jar --paramDetails " + params.getIds().get(0) : "");
                ...
                if (jobid != null) {
                    ...
                    if (cli.hasOption("async")) {
                        ...
                        printDebugMessage("run", String.format("You can check status of your job with:\n" +
                                                 "java -jar mview.jar --status --jobid %s\n", jobid), 11);

                        if (outputLevel > 0)
                            System.out.println("To check status: java -jar mview.jar --status --jobid " + jobid);
                    ...
                    // Synchronous execution
                        ...
                        if (ClientUtilsmview.getStatusInIntervals(this, jobid, pollFreq, outputLevel, debugLevel)) {
    ...
    public void downloadResults(CommandLine cli, String jobid) throws IOException {
        ...
        if (cli.hasOption("outformat")) {
            resultTypes = ClientUtilsmview.processUserOutputFormats(cli.getOptionValue("outformat"),
