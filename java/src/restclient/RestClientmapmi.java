...
//
// MapMi (REST) web service Java client.
...
public class RestClientmapmi {
    ...
    private final String generalInfo = "EMBL-EBI MapMi Java Client:\n"
                                   + "\n"
                                   ...
                                   + "\n[Optional]\n"
                                   + "  --scorethr            Score threshold. The minimum score for a candidate hairpi.\n"
                                   ...
                                   + "  Usage: java -jar mapmi.jar --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: results as an attachment\n"
                                   + "\n"
                                   + "Asynchronous job:\n"
                                   + "  Use this if you want to retrieve the results at a later time. The results\n"
                                   + "  are stored for up to 24 hours.\n"
                                   + "  Usage: java -jar mapmi.jar --async --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: jobid\n"
                                   + "\n"
                                   + "Check status of Asynchronous job:\n"
                                   + "   Usage: java -jar mapmi.jar --status --jobid <jobId>\n"
                                   + "\n"
                                   + "Retrieve job data:\n"
                                   + "  Use the jobid to query for the status of the job. If the job is finished,\n"
                                   + "  it also returns the results/errors.\n"
                                   + "  Usage: java -jar mapmi.jar --polljob --jobid <jobId> [--outfile string]\n"
    ...
    public static void main(String[] args) throws IOException, InterruptedException, ServiceException, JAXBException {
        ...
        RestClientmapmi client = new RestClientmapmi();
        client.run(args);
    }
     ...
     */
    private RestClientmapmi() throws IOException {

        toolId = "mapmi";
        toolName = "MapMi";
        toolDescription = "RNA analysis with MapMi.";
    ...
    private void addGeneralOptions() {
        allOptions.addOption("scorethr", "", true, "Score threshold. The minimum score for a candidate hairpin.");
        allOptions.addOption("longext", "", true, "Long match extention (bp).");
        allOptions.addOption("shortext", "", true, "Short match extention (bp).");
        allOptions.addOption("maxmis", "", true, "Maximum mature mismatches.");
        allOptions.addOption("mismatchpen", "", true, "Mature mismatch penalty.");
        allOptions.addOption("bowtie", "", true, "Maximum allowed matches per candidate genome (Bowtie-m). 0 for"
                   + "disable.");
        allOptions.addOption("excludecan", "", false, "Exclude candidates in loop. Binary toggle to exclude mature.");
        allOptions.addOption("maxloop", "", true, "Maximum loop overlap. Maximum allowed mature basepairs overlapping"
                   + "loop.");
        allOptions.addOption("sequence", "", true, "The query sequence(s) can be entered directly into this form. The"
                   ...
                   + "entry.");
        allOptions.addOption("metazoa_species", "", true, "Ensembl Metazoa Species");
        allOptions.addOption("ensembl_species", "", true, "Ensembl Species");
    ...
    public WsParameters getParams() {
        ...
        if (ClientUtilsmapmi.isResponseCorrect(response, debugLevel)) {
            ...
            if (cli.hasOption("params")) {
                ...
                if (params != null) {
                    ...
                    sb.append("\n\nTo learn more about selected parameter run:\n")
                            .append("java -jar ").append("mapmi.jar")
                            .append(" --paramDetails <parameter-name>")
                            .append(params.getIds().size() > 0 ? "\n\nFor example:\njava -jar mapmi.jar --paramDetails " + params.getIds().get(0) : "");
                ...
                if (jobid != null) {
                    ...
                    if (cli.hasOption("async")) {
                        ...
                        printDebugMessage("run", String.format("You can check status of your job with:\n" +
                                                 "java -jar mapmi.jar --status --jobid %s\n", jobid), 11);
                    ...
                    // Synchronous execution
                        ...
                        if (ClientUtilsmapmi.getStatusInIntervals(this, jobid, pollFreq, outputLevel, debugLevel)) {
    ...
    public void downloadResults(CommandLine cli, String jobid) throws IOException {
        ...
        if (cli.hasOption("outformat")) {
            resultTypes = ClientUtilsmapmi.processUserOutputFormats(cli.getOptionValue("outformat"),
