...
//
// HMMER 3 phmmer (REST) web service Java client.
...
public class RestClienthmmer3_phmmer {

    static {
        System.setProperty("org.slf4j.simpleLogger.defaultLogLevel", "ERROR");
    }

    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(RestClienthmmer3_phmmer.class);
    ...
    private final String generalInfo = "EMBL-EBI HMMER 3 phmmer Java Client:\n"
                                   + "\n"
                                   ...
                                   + "  Usage: java -jar hmmer3_phmmer.jar --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: results as an attachment\n"
                                   + "\n"
                                   + "Asynchronous job:\n"
                                   + "  Use this if you want to retrieve the results at a later time. The results\n"
                                   + "  are stored for up to 24 hours.\n"
                                   + "  Usage: java -jar hmmer3_phmmer.jar --async --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: jobid\n"
                                   + "\n"
                                   + "Check status of Asynchronous job:\n"
                                   + "   Usage: java -jar hmmer3_phmmer.jar --status --jobid <jobId>\n"
                                   + "\n"
                                   + "Retrieve job data:\n"
                                   + "  Use the jobid to query for the status of the job. If the job is finished,\n"
                                   + "  it also returns the results/errors.\n"
                                   + "  Usage: java -jar hmmer3_phmmer.jar --polljob --jobid <jobId> [--outfile string]\n"
    ...
    public static void main(String[] args) throws IOException, InterruptedException, ServiceException, JAXBException {
        ...
        RestClienthmmer3_phmmer client = new RestClienthmmer3_phmmer();
        client.run(args);
    }
     ...
     */
    private RestClienthmmer3_phmmer() throws IOException {

        toolId = "hmmer3_phmmer";
        toolName = "HMMER 3 phmmer";
        toolDescription = "Protein function analysis with HMMER 3 phmmer.";
    ...
    public void printGetStatus(String jobid) throws IOException, ServiceException {
        ...
        if (outputLevel > 0 && status.equals("FINISHED")) {
             System.out.println("To get results: java -jar hmmer3_phmmer.jar --polljob --jobid " + jobid);
        }
    }
    ...
    private void addGeneralOptions() {
        allOptions.addOption("incE", "", true, "Significance E-values[Sequence]");
        allOptions.addOption("incdomE", "", true, "Significance E-values[Hit]");
        allOptions.addOption("E", "", true, "Report E-values[Sequence]");
        allOptions.addOption("domE", "", true, "Report E-values[Hit]");
        allOptions.addOption("incT", "", true, "Significance bit scores[Sequence]");
        allOptions.addOption("incdomT", "", true, "Significance bit scores[Hit]");
        allOptions.addOption("T", "", true, "Report bit scores[Sequence]");
        allOptions.addOption("domT", "", true, "Report bit scores[Hit]");
        allOptions.addOption("popen", "", true, "Gap Penalties[open]");
        allOptions.addOption("pextend", "", true, "Gap Penalties[extend]");
        allOptions.addOption("mx", "", true, "Gap Penalties[Substitution scoring matrix]");
        allOptions.addOption("nobias", "", false, "Filters");
        allOptions.addOption("alignView", "", false, "Output alignment in result");
        allOptions.addOption("database", "", true, "Sequence Database");
        allOptions.addOption("evalue", "", true, "Expectation value cut-off for reporting target profiles in the per-"
                   + "target output.");
        allOptions.addOption("sequence", "", true, "The input sequence can be entered directly into this form. The"
    ...
    public WsParameters getParams() {
        ...
        if (ClientUtilshmmer3_phmmer.isResponseCorrect(response, debugLevel)) {
        ...
        try {
            ...
            if (cli.hasOption("params")) {
                ...
                if (params != null) {
                    ...
                    sb.append("\n\nTo learn more about selected parameter run:\n")
                            .append("java -jar ").append("hmmer3_phmmer.jar")
                            .append(" --paramDetails <parameter-name>")
                            .append(params.getIds().size() > 0 ? "\n\nFor example:\njava -jar hmmer3_phmmer.jar --paramDetails " + params.getIds().get(0) : "");
            ...
            // Details of selected parameter
                ...
                if (parameter != null) {
                    ...
                    ClientUtilshmmer3_phmmer.marshallToXML(parameter, debugLevel, "parameters");
                } else {
            ...
            // Submit new job
                ...
                String jobid = null;
                if (sequence != null){
                    jobid = submitJob(cli, ClientUtilshmmer3_phmmer.loadData(sequence), null);
                } else if (asequence != null && bsequence != null) {
                    jobid = submitJob(cli, ClientUtilshmmer3_phmmer.loadData(asequence),
                                             ClientUtilshmmer3_phmmer.loadData(bsequence));
                }
                ...
                if (jobid != null) {
                    ...
                    if (cli.hasOption("async")) {
                        ...
                        printDebugMessage("run", String.format("You can check status of your job with:\n" +
                                                 "java -jar hmmer3_phmmer.jar --status --jobid %s\n", jobid), 11);

                        if (outputLevel > 0)
                            System.out.println("To check status: java -jar hmmer3_phmmer.jar --status --jobid " + jobid);
                    ...
                    // Synchronous execution
                        ...
                        if (ClientUtilshmmer3_phmmer.getStatusInIntervals(this, jobid, pollFreq, outputLevel, debugLevel)) {
            ...
            // Check already submitted job
                ...
                if (cli.hasOption("resultTypes")) {
                    ...
                    ClientUtilshmmer3_phmmer.marshallToXML(getResultTypesForJobId(jobid), debugLevel, "types");
                    if (outputLevel > 0)
                        System.out.println("To get results:\n  java -jar hmmer3_phmmer.jar --polljob --jobid " + jobid
                                           + "\n  java -jar hmmer3_phmmer.jar --polljob --outformat <type> --jobid " + jobid);
                }
    ...
    public void downloadResults(CommandLine cli, String jobid) throws IOException {
        ...
        if (cli.hasOption("outformat")) {
            resultTypes = ClientUtilshmmer3_phmmer.processUserOutputFormats(cli.getOptionValue("outformat"),
    ...
    private String submitJob(CommandLine cli, String inputSeq, String inputSeq2)
        ...
        if (cli.hasOption("mx") == false)
           form.putSingle("mx", "BLOSUM62");
