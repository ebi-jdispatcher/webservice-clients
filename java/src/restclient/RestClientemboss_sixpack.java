...
//
// EMBOSS sixpack (REST) web service Java client.
...
public class RestClientemboss_sixpack {

    static {
        System.setProperty("org.slf4j.simpleLogger.defaultLogLevel", "ERROR");
    }

    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(RestClientemboss_sixpack.class);
    ...
    private final String generalInfo = "EMBL-EBI EMBOSS sixpack Java Client:\n"
                                   + "\n"
                                   ...
                                   + "  Usage: java -jar emboss_sixpack.jar --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: results as an attachment\n"
                                   + "\n"
                                   + "Asynchronous job:\n"
                                   + "  Use this if you want to retrieve the results at a later time. The results\n"
                                   + "  are stored for up to 24 hours.\n"
                                   + "  Usage: java -jar emboss_sixpack.jar --async --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: jobid\n"
                                   + "\n"
                                   + "Check status of Asynchronous job:\n"
                                   + "   Usage: java -jar emboss_sixpack.jar --status --jobid <jobId>\n"
                                   + "\n"
                                   + "Retrieve job data:\n"
                                   + "  Use the jobid to query for the status of the job. If the job is finished,\n"
                                   + "  it also returns the results/errors.\n"
                                   + "  Usage: java -jar emboss_sixpack.jar --polljob --jobid <jobId> [--outfile string]\n"
    ...
    public static void main(String[] args) throws IOException, InterruptedException, ServiceException, JAXBException {
        ...
        RestClientemboss_sixpack client = new RestClientemboss_sixpack();
        client.run(args);
    }
     ...
     */
    private RestClientemboss_sixpack() throws IOException {

        toolId = "emboss_sixpack";
        toolName = "EMBOSS sixpack";
        toolDescription = "Sequence translations with sixpack.";
    ...
    public void printGetStatus(String jobid) throws IOException, ServiceException {
        ...
        if (outputLevel > 0 && status.equals("FINISHED")) {
             System.out.println("To get results: java -jar emboss_sixpack.jar --polljob --jobid " + jobid);
        }
    }
    ...
    private void addGeneralOptions() {
        ...
        allOptions.addOption("firstorf", "", false, "Count the beginning of a sequence as a possible ORF, even if its"
                   + "inferior to the minimal ORF size.");
        allOptions.addOption("lastorf", "", false, "Count the end of a sequence as a possible ORF, even if its not"
                   + "finishing with a STOP, or inferior to the minimal ORF size.");
        allOptions.addOption("reverse", "", false, "Choose this option if you wish to reverse and compliment your"
                   + "sequence.");
        allOptions.addOption("orfminsize", "", true, "Minimum size of Open Reading Frames (ORFs) to display in the"
                   + "translations.");
        allOptions.addOption("sequence", "", true, "A DNA sequence can be entered or pasted into this box. Ideally use a"
    ...
    public WsParameters getParams() {
        ...
        if (ClientUtilsemboss_sixpack.isResponseCorrect(response, debugLevel)) {
        ...
        try {
            ...
            if (cli.hasOption("params")) {
                ...
                if (params != null) {
                    ...
                    sb.append("\n\nTo learn more about selected parameter run:\n")
                            .append("java -jar ").append("emboss_sixpack.jar")
                            .append(" --paramDetails <parameter-name>")
                            .append(params.getIds().size() > 0 ? "\n\nFor example:\njava -jar emboss_sixpack.jar --paramDetails " + params.getIds().get(0) : "");
            ...
            // Details of selected parameter
                ...
                if (parameter != null) {
                    ...
                    ClientUtilsemboss_sixpack.marshallToXML(parameter, debugLevel, "parameters");
                } else {
            ...
            // Submit new job
                ...
                String jobid = null;
                if (sequence != null){
                    jobid = submitJob(cli, ClientUtilsemboss_sixpack.loadData(sequence), null);
                } else if (asequence != null && bsequence != null) {
                    jobid = submitJob(cli, ClientUtilsemboss_sixpack.loadData(asequence),
                                             ClientUtilsemboss_sixpack.loadData(bsequence));
                }
                ...
                if (jobid != null) {
                    ...
                    if (cli.hasOption("async")) {
                        ...
                        printDebugMessage("run", String.format("You can check status of your job with:\n" +
                                                 "java -jar emboss_sixpack.jar --status --jobid %s\n", jobid), 11);

                        if (outputLevel > 0)
                            System.out.println("To check status: java -jar emboss_sixpack.jar --status --jobid " + jobid);
                    ...
                    // Synchronous execution
                        ...
                        if (ClientUtilsemboss_sixpack.getStatusInIntervals(this, jobid, pollFreq, outputLevel, debugLevel)) {
            ...
            // Check already submitted job
                ...
                if (cli.hasOption("resultTypes")) {
                    ...
                    ClientUtilsemboss_sixpack.marshallToXML(getResultTypesForJobId(jobid), debugLevel, "types");
                    if (outputLevel > 0)
                        System.out.println("To get results:\n  java -jar emboss_sixpack.jar --polljob --jobid " + jobid
                                           + "\n  java -jar emboss_sixpack.jar --polljob --outformat <type> --jobid " + jobid);
                }
    ...
    public void downloadResults(CommandLine cli, String jobid) throws IOException {
        ...
        if (cli.hasOption("outformat")) {
            resultTypes = ClientUtilsemboss_sixpack.processUserOutputFormats(cli.getOptionValue("outformat"),
    ...
    private String submitJob(CommandLine cli, String inputSeq, String inputSeq2)
        ...
        if (cli.hasOption("firstorf") == false)
           form.putSingle("firstorf", "true");

        if (cli.hasOption("lastorf") == false)
           form.putSingle("lastorf", "true");

        if (cli.hasOption("reverse") == false)
           form.putSingle("reverse", "true");
