...
//
// NCBI Blast (REST) web service Java client.
...
public class RestClientncbiblast {
    ...
    private final String generalInfo = "EMBL-EBI NCBI Blast Java Client:\n"
                                   + "\n"
                                   ...
                                   + "  --transltable         Query Genetic code to use in translatio.\n"
                                   ...
                                   + "  Usage: java -jar ncbiblast.jar --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: results as an attachment\n"
                                   + "\n"
                                   + "Asynchronous job:\n"
                                   + "  Use this if you want to retrieve the results at a later time. The results\n"
                                   + "  are stored for up to 24 hours.\n"
                                   + "  Usage: java -jar ncbiblast.jar --async --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: jobid\n"
                                   + "\n"
                                   + "Check status of Asynchronous job:\n"
                                   + "   Usage: java -jar ncbiblast.jar --status --jobid <jobId>\n"
                                   + "\n"
                                   + "Retrieve job data:\n"
                                   + "  Use the jobid to query for the status of the job. If the job is finished,\n"
                                   + "  it also returns the results/errors.\n"
                                   + "  Usage: java -jar ncbiblast.jar --polljob --jobid <jobId> [--outfile string]\n"
    ...
    public static void main(String[] args) throws IOException, InterruptedException, ServiceException, JAXBException {
        ...
        RestClientncbiblast client = new RestClientncbiblast();
        client.run(args);
    }
     ...
     */
    private RestClientncbiblast() throws IOException {

        toolId = "ncbiblast";
        toolName = "NCBI Blast";
        toolDescription = "Sequence similarity search with NCBI Blast.";
    ...
    private void addGeneralOptions() {
        allOptions.addOption("program", "", true, "The BLAST program to be used for the Sequence Similarity Search.");
        allOptions.addOption("task", "", true, "Task option (only selectable for blastn)");
        allOptions.addOption("matrix", "", true, "(Protein searches) The substitution matrix used for scoring alignments"
        ...
        allOptions.addOption("exp", "", true, "Limits the number of scores and alignments reported based on the"
                   + "expectation value. This is the maximum number of times the match is"
                   + "expected to occur by chance.");
        allOptions.addOption("dropoff", "", true, "The amount a score can drop before gapped extension of word hits is"
                   + "halted");
        allOptions.addOption("match_scores", "", true, "(Nucleotide searches) The match score is the bonus to the alignment"
        ...
        allOptions.addOption("gapalign", "", false, "This is a true/false setting that tells the program the perform"
        ...
        allOptions.addOption("compstats", "", true, "Use composition-based statistics.");
        allOptions.addOption("align", "", true, "Formating for the alignments");
        allOptions.addOption("transltable", "", true, "Query Genetic code to use in translation");
        allOptions.addOption("stype", "", true, "Indicates if the sequence is protein or DNA/RNA.");
        ...
        allOptions.addOption("database", "", true, "Database");
    ...
    public WsParameters getParams() {
        ...
        if (ClientUtilsncbiblast.isResponseCorrect(response, debugLevel)) {
            ...
            if (cli.hasOption("params")) {
                ...
                if (params != null) {
                    ...
                    sb.append("\n\nTo learn more about selected parameter run:\n")
                            .append("java -jar ").append("ncbiblast.jar")
                            .append(" --paramDetails <parameter-name>")
                            .append(params.getIds().size() > 0 ? "\n\nFor example:\njava -jar ncbiblast.jar --paramDetails " + params.getIds().get(0) : "");
                ...
                if (jobid != null) {
                    ...
                    if (cli.hasOption("async")) {
                        ...
                        printDebugMessage("run", String.format("You can check status of your job with:\n" +
                                                 "java -jar ncbiblast.jar --status --jobid %s\n", jobid), 11);
                    ...
                    // Synchronous execution
                        ...
                        if (ClientUtilsncbiblast.getStatusInIntervals(this, jobid, pollFreq, outputLevel, debugLevel)) {
    ...
    public void downloadResults(CommandLine cli, String jobid) throws IOException {
        ...
        if (cli.hasOption("outformat")) {
            resultTypes = ClientUtilsncbiblast.processUserOutputFormats(cli.getOptionValue("outformat"),
