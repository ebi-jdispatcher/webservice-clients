...
//
// EMBOSS polydot (REST) web service Java client.
...
public class RestClientemboss_polydot {

    static {
        System.setProperty("org.slf4j.simpleLogger.defaultLogLevel", "ERROR");
    }

    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(RestClientemboss_polydot.class);
    ...
    private final String generalInfo = "EMBL-EBI EMBOSS polydot Java Client:\n"
                                   + "\n"
                                   ...
                                   + "  Usage: java -jar emboss_polydot.jar --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: results as an attachment\n"
                                   + "\n"
                                   + "Asynchronous job:\n"
                                   + "  Use this if you want to retrieve the results at a later time. The results\n"
                                   + "  are stored for up to 24 hours.\n"
                                   + "  Usage: java -jar emboss_polydot.jar --async --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: jobid\n"
                                   + "\n"
                                   + "Check status of Asynchronous job:\n"
                                   + "   Usage: java -jar emboss_polydot.jar --status --jobid <jobId>\n"
                                   + "\n"
                                   + "Retrieve job data:\n"
                                   + "  Use the jobid to query for the status of the job. If the job is finished,\n"
                                   + "  it also returns the results/errors.\n"
                                   + "  Usage: java -jar emboss_polydot.jar --polljob --jobid <jobId> [--outfile string]\n"
    ...
    public static void main(String[] args) throws IOException, InterruptedException, ServiceException, JAXBException {
        ...
        RestClientemboss_polydot client = new RestClientemboss_polydot();
        client.run(args);
    }
     ...
     */
    private RestClientemboss_polydot() throws IOException {

        toolId = "emboss_polydot";
        toolName = "EMBOSS polydot";
        toolDescription = "Sequence statistics and plots with polydot.";
    ...
    public void printGetStatus(String jobid) throws IOException, ServiceException {
        ...
        if (outputLevel > 0 && status.equals("FINISHED")) {
             System.out.println("To get results: java -jar emboss_polydot.jar --polljob --jobid " + jobid);
        }
    }
    ...
    private void addGeneralOptions() {
        ...
        allOptions.addOption("sequence", "", true, "Two or more aligned sequences are required. There is currently a"
                   + "sequence input limit of 500 sequences and 1MB of data.");
        allOptions.addOption("wordsize", "", true, "Word size.");
        allOptions.addOption("gap", "", true, "This specifies the size of the gap that is used to separate the"
                   + "individual dotplots in the display. The size is measured in residues,"
                   + "as displayed in the output.");
        allOptions.addOption("boxit", "", false, "Draw a box around dotplot.");
    ...
    public WsParameters getParams() {
        ...
        if (ClientUtilsemboss_polydot.isResponseCorrect(response, debugLevel)) {
        ...
        try {
            ...
            if (cli.hasOption("params")) {
                ...
                if (params != null) {
                    ...
                    sb.append("\n\nTo learn more about selected parameter run:\n")
                            .append("java -jar ").append("emboss_polydot.jar")
                            .append(" --paramDetails <parameter-name>")
                            .append(params.getIds().size() > 0 ? "\n\nFor example:\njava -jar emboss_polydot.jar --paramDetails " + params.getIds().get(0) : "");
            ...
            // Details of selected parameter
                ...
                if (parameter != null) {
                    ...
                    ClientUtilsemboss_polydot.marshallToXML(parameter, debugLevel, "parameters");
                } else {
            ...
            // Submit new job
                ...
                String jobid = null;
                if (sequence != null){
                    jobid = submitJob(cli, ClientUtilsemboss_polydot.loadData(sequence), null);
                } else if (asequence != null && bsequence != null) {
                    jobid = submitJob(cli, ClientUtilsemboss_polydot.loadData(asequence),
                                             ClientUtilsemboss_polydot.loadData(bsequence));
                }
                ...
                if (jobid != null) {
                    ...
                    if (cli.hasOption("async")) {
                        ...
                        printDebugMessage("run", String.format("You can check status of your job with:\n" +
                                                 "java -jar emboss_polydot.jar --status --jobid %s\n", jobid), 11);

                        if (outputLevel > 0)
                            System.out.println("To check status: java -jar emboss_polydot.jar --status --jobid " + jobid);
                    ...
                    // Synchronous execution
                        ...
                        if (ClientUtilsemboss_polydot.getStatusInIntervals(this, jobid, pollFreq, outputLevel, debugLevel)) {
            ...
            // Check already submitted job
                ...
                if (cli.hasOption("resultTypes")) {
                    ...
                    ClientUtilsemboss_polydot.marshallToXML(getResultTypesForJobId(jobid), debugLevel, "types");
                    if (outputLevel > 0)
                        System.out.println("To get results:\n  java -jar emboss_polydot.jar --polljob --jobid " + jobid
                                           + "\n  java -jar emboss_polydot.jar --polljob --outformat <type> --jobid " + jobid);
                }
    ...
    public void downloadResults(CommandLine cli, String jobid) throws IOException {
        ...
        if (cli.hasOption("outformat")) {
            resultTypes = ClientUtilsemboss_polydot.processUserOutputFormats(cli.getOptionValue("outformat"),
    ...
    private String submitJob(CommandLine cli, String inputSeq, String inputSeq2)
        ...
        if (cli.hasOption("boxit") == false)
           form.putSingle("boxit", "true");
