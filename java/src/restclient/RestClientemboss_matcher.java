...
//
// EMBOSS matcher (REST) web service Java client.
...
public class RestClientemboss_matcher {

    static {
        System.setProperty("org.slf4j.simpleLogger.defaultLogLevel", "ERROR");
    }

    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(RestClientemboss_matcher.class);
    ...
    private final String generalInfo = "EMBL-EBI EMBOSS matcher Java Client:\n"
                                   + "\n"
                                   ...
                                   + "  Usage: java -jar emboss_matcher.jar --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: results as an attachment\n"
                                   + "\n"
                                   + "Asynchronous job:\n"
                                   + "  Use this if you want to retrieve the results at a later time. The results\n"
                                   + "  are stored for up to 24 hours.\n"
                                   + "  Usage: java -jar emboss_matcher.jar --async --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: jobid\n"
                                   + "\n"
                                   + "Check status of Asynchronous job:\n"
                                   + "   Usage: java -jar emboss_matcher.jar --status --jobid <jobId>\n"
                                   + "\n"
                                   + "Retrieve job data:\n"
                                   + "  Use the jobid to query for the status of the job. If the job is finished,\n"
                                   + "  it also returns the results/errors.\n"
                                   + "  Usage: java -jar emboss_matcher.jar --polljob --jobid <jobId> [--outfile string]\n"
    ...
    public static void main(String[] args) throws IOException, InterruptedException, ServiceException, JAXBException {
        ...
        RestClientemboss_matcher client = new RestClientemboss_matcher();
        client.run(args);
    }
     ...
     */
    private RestClientemboss_matcher() throws IOException {

        toolId = "emboss_matcher";
        toolName = "EMBOSS matcher";
        toolDescription = "Pairwise sequence alignment with Matcher.";
    ...
    public void printGetStatus(String jobid) throws IOException, ServiceException {
        ...
        if (outputLevel > 0 && status.equals("FINISHED")) {
             System.out.println("To get results: java -jar emboss_matcher.jar --polljob --jobid " + jobid);
        }
    }
    ...
    private void addGeneralOptions() {
        ...
        allOptions.addOption("alternatives", "", true, "Show additional alignments");
    ...
    public WsParameters getParams() {
        ...
        if (ClientUtilsemboss_matcher.isResponseCorrect(response, debugLevel)) {
        ...
        try {
            ...
            if (cli.hasOption("params")) {
                ...
                if (params != null) {
                    ...
                    sb.append("\n\nTo learn more about selected parameter run:\n")
                            .append("java -jar ").append("emboss_matcher.jar")
                            .append(" --paramDetails <parameter-name>")
                            .append(params.getIds().size() > 0 ? "\n\nFor example:\njava -jar emboss_matcher.jar --paramDetails " + params.getIds().get(0) : "");
            ...
            // Details of selected parameter
                ...
                if (parameter != null) {
                    ...
                    ClientUtilsemboss_matcher.marshallToXML(parameter, debugLevel, "parameters");
                } else {
            ...
            // Submit new job
                ...
                String jobid = null;
                if (sequence != null){
                    jobid = submitJob(cli, ClientUtilsemboss_matcher.loadData(sequence), null);
                } else if (asequence != null && bsequence != null) {
                    jobid = submitJob(cli, ClientUtilsemboss_matcher.loadData(asequence),
                                             ClientUtilsemboss_matcher.loadData(bsequence));
                }
                ...
                if (jobid != null) {
                    ...
                    if (cli.hasOption("async")) {
                        ...
                        printDebugMessage("run", String.format("You can check status of your job with:\n" +
                                                 "java -jar emboss_matcher.jar --status --jobid %s\n", jobid), 11);

                        if (outputLevel > 0)
                            System.out.println("To check status: java -jar emboss_matcher.jar --status --jobid " + jobid);
                    ...
                    // Synchronous execution
                        ...
                        if (ClientUtilsemboss_matcher.getStatusInIntervals(this, jobid, pollFreq, outputLevel, debugLevel)) {
            ...
            // Check already submitted job
                ...
                if (cli.hasOption("resultTypes")) {
                    ...
                    ClientUtilsemboss_matcher.marshallToXML(getResultTypesForJobId(jobid), debugLevel, "types");
                    if (outputLevel > 0)
                        System.out.println("To get results:\n  java -jar emboss_matcher.jar --polljob --jobid " + jobid
                                           + "\n  java -jar emboss_matcher.jar --polljob --outformat <type> --jobid " + jobid);
                }
    ...
    public void downloadResults(CommandLine cli, String jobid) throws IOException {
        ...
        if (cli.hasOption("outformat")) {
            resultTypes = ClientUtilsemboss_matcher.processUserOutputFormats(cli.getOptionValue("outformat"),
    ...
    private String submitJob(CommandLine cli, String inputSeq, String inputSeq2)
        ...
        if (cli.hasOption("gapext") == false)
           form.putSingle("gapext", "4");

        if (cli.hasOption("alternatives") == false)
           form.putSingle("alternatives", "1");
