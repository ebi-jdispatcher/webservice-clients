...
//
// EMBOSS newcpgreport (REST) web service Java client.
...
public class RestClientemboss_newcpgreport {
    ...
    private final String generalInfo = "EMBL-EBI EMBOSS newcpgreport Java Client:\n"
                                   + "\n"
                                   + "Sequence statistics and plots with newcpgreport.\n"
                                   + "\n"
                                   + "[Required (for job submission)]\n"
                                   + "  --email               E-mail address.\n"
                                   + "  --sequence            The sequence to be analysed can be entered directly into\n"
                                   + "                        this form. Sequences can be in GCG, FASTA, EMBL, GenBank,\n"
                                   + "                        PIR, NBRF, or PHYLIP format. Partially formatted sequences\n"
                                   + "                        are not accepted.\n"
                                   ...
                                   + "                        https://www.ebi.ac.uk/Tools/services/rest/emboss_newcpgreport\n"
                                   + "\n"
                                   + "Synchronous job:\n"
                                   + "  The results/errors are returned as soon as the job is finished.\n"
                                   + "  Usage: java -jar emboss_newcpgreport.jar --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: results as an attachment\n"
                                   + "\n"
                                   + "Asynchronous job:\n"
                                   + "  Use this if you want to retrieve the results at a later time. The results\n"
                                   + "  are stored for up to 24 hours.\n"
                                   + "  Usage: java -jar emboss_newcpgreport.jar --async --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: jobid\n"
                                   + "\n"
                                   + "Check status of Asynchronous job:\n"
                                   + "   Usage: java -jar emboss_newcpgreport.jar --status --jobid <jobId>\n"
                                   + "\n"
                                   + "Retrieve job data:\n"
                                   + "  Use the jobid to query for the status of the job. If the job is finished,\n"
                                   + "  it also returns the results/errors.\n"
                                   + "  Usage: java -jar emboss_newcpgreport.jar --polljob --jobid <jobId> [--outfile string]\n"
    ...
    public static void main(String[] args) throws IOException, InterruptedException, ServiceException, JAXBException {
        ...
        RestClientemboss_newcpgreport client = new RestClientemboss_newcpgreport();
        client.run(args);
    }
     ...
     */
    private RestClientemboss_newcpgreport() throws IOException {

        toolId = "emboss_newcpgreport";
        toolName = "EMBOSS newcpgreport";
        toolDescription = "Sequence statistics and plots with newcpgreport.";
    ...
    public void printGetStatus(String jobid) throws IOException, ServiceException {
        ...
        if (outputLevel > 0 && status.equals("FINISHED")) {
             System.out.println("To get results: java -jar emboss_newcpgreport.jar --polljob --jobid " + jobid);
        }
    }
    ...
    private void addGeneralOptions() {
        allOptions.addOption("sequence", "", true, "The sequence to be analysed can be entered directly into this form."
                   + "Sequences can be in GCG, FASTA, EMBL, GenBank, PIR, NBRF, or PHYLIP"
                   + "format. Partially formatted sequences are not accepted.");
        allOptions.addOption("window", "", true, "The percentage CG content and the Observed frequency of CG is"
        ...
        allOptions.addOption("minlen", "", true, "This sets the minimum length that a CpG island has to be before it is"
                   + "reported.");
        allOptions.addOption("minoe", "", true, "This sets the minimum average observed to expected ratio of C plus G"
                   + "to CpG in a set of 10 windows that are required before a CpG island is"
                   + "reported.");
        allOptions.addOption("minpc", "", true, "This sets the minimum average percentage of G plus C a set of 10"
    ...
    public WsParameters getParams() {
        ...
        if (ClientUtilsemboss_newcpgreport.isResponseCorrect(response, debugLevel)) {
        ...
        try {
            ...
            if (cli.hasOption("params")) {
                ...
                if (params != null) {
                    ...
                    sb.append("\n\nTo learn more about selected parameter run:\n")
                            .append("java -jar ").append("emboss_newcpgreport.jar")
                            .append(" --paramDetails <parameter-name>")
                            .append(params.getIds().size() > 0 ? "\n\nFor example:\njava -jar emboss_newcpgreport.jar --paramDetails " + params.getIds().get(0) : "");
            ...
            // Details of selected parameter
                ...
                if (parameter != null) {
                    ...
                    ClientUtilsemboss_newcpgreport.marshallToXML(parameter, debugLevel, "parameters");
                } else {
            ...
            // Submit new job
                ...
                String jobid = null;
                if (sequence != null){
                    jobid = submitJob(cli, ClientUtilsemboss_newcpgreport.loadData(sequence), null);
                } else if (asequence != null && bsequence != null) {
                    jobid = submitJob(cli, ClientUtilsemboss_newcpgreport.loadData(asequence),
                                             ClientUtilsemboss_newcpgreport.loadData(bsequence));
                }
                ...
                if (jobid != null) {
                    ...
                    if (cli.hasOption("async")) {
                        ...
                        printDebugMessage("run", String.format("You can check status of your job with:\n" +
                                                 "java -jar emboss_newcpgreport.jar --status --jobid %s\n", jobid), 11);
                    ...
                    // Synchronous execution
                        ...
                        if (ClientUtilsemboss_newcpgreport.getStatusInIntervals(this, jobid, pollFreq, outputLevel, debugLevel)) {
            ...
            // Check already submitted job
                ...
                if (cli.hasOption("resultTypes")) {
                    ...
                    ClientUtilsemboss_newcpgreport.marshallToXML(getResultTypesForJobId(jobid), debugLevel, "types");
                    if (outputLevel > 0)
                        System.out.println("To get results:\n  java -jar emboss_newcpgreport.jar --polljob --jobid " + jobid
                                           + "\n  java -jar emboss_newcpgreport.jar --polljob --outformat <type> --jobid " + jobid);
                }
    ...
    public void downloadResults(CommandLine cli, String jobid) throws IOException {
        ...
        if (cli.hasOption("outformat")) {
            resultTypes = ClientUtilsemboss_newcpgreport.processUserOutputFormats(cli.getOptionValue("outformat"),
