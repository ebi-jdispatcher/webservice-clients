...
//
// Clustal Omega (REST) web service Java client.
...
public class RestClientclustalo {

    static {
        System.setProperty("org.slf4j.simpleLogger.defaultLogLevel", "ERROR");
    }

    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(RestClientclustalo.class);
    ...
    private final String generalInfo = "EMBL-EBI Clustal Omega Java Client:\n"
                                   ...
                                   + "  Usage: java -jar clustalo.jar --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: results as an attachment\n"
                                   + "\n"
                                   + "Asynchronous job:\n"
                                   + "  Use this if you want to retrieve the results at a later time. The results\n"
                                   + "  are stored for up to 24 hours.\n"
                                   + "  Usage: java -jar clustalo.jar --async --email <your@email.com> [options...] <SeqFile|SeqID(s)>\n"
                                   + "  Returns: jobid\n"
                                   + "\n"
                                   + "Check status of Asynchronous job:\n"
                                   + "   Usage: java -jar clustalo.jar --status --jobid <jobId>\n"
                                   + "\n"
                                   + "Retrieve job data:\n"
                                   + "  Use the jobid to query for the status of the job. If the job is finished,\n"
                                   + "  it also returns the results/errors.\n"
                                   + "  Usage: java -jar clustalo.jar --polljob --jobid <jobId> [--outfile string]\n"
    ...
    public static void main(String[] args) throws IOException, InterruptedException, ServiceException, JAXBException {
        ...
        RestClientclustalo client = new RestClientclustalo();
        client.run(args);
    }
     ...
     */
    private RestClientclustalo() throws IOException {

        toolId = "clustalo";
        toolName = "Clustal Omega";
        toolDescription = "Multiple sequence alignment with Clustal Omega.";
    ...
    public void printGetStatus(String jobid) throws IOException, ServiceException {
        ...
        if (outputLevel > 0 && status.equals("FINISHED")) {
             System.out.println("To get results: java -jar clustalo.jar --polljob --jobid " + jobid);
        }
    }
    ...
    private void addGeneralOptions() {
        allOptions.addOption("guidetreeout", "", false, "Output guide tree.");
        allOptions.addOption("dismatout", "", false, "Output distance matrix. This is only calculated if the mBed-like"
                   + "clustering guide tree is set to false.");
        allOptions.addOption("dealign", "", false, "Remove any existing alignment (gaps) from input sequences.");
        allOptions.addOption("mbed", "", false, "This option uses a sample of the input sequences and then represents"
                   ...
                   + "is large.");
        allOptions.addOption("mbediteration", "", false, "Use mBed-like clustering during subsequent iterations.");
        allOptions.addOption("iterations", "", true, "Number of (combined guide-tree/HMM) iterations.");
        allOptions.addOption("gtiterations", "", true, "Having set the number of combined iterations, this parameter can be"
                   + "changed to limit the number of guide tree iterations within the"
                   + "combined iterations.");
        allOptions.addOption("hmmiterations", "", true, "Having set the number of combined iterations, this parameter can be"
                   + "changed to limit the number of HMM iterations within the combined"
                   + "iterations.");
        allOptions.addOption("outfmt", "", true, "Format for generated multiple sequence alignment.");
    ...
    public WsParameters getParams() {
        ...
        if (ClientUtilsclustalo.isResponseCorrect(response, debugLevel)) {
        ...
        try {
            ...
            if (cli.hasOption("params")) {
                ...
                if (params != null) {
                    ...
                    sb.append("\n\nTo learn more about selected parameter run:\n")
                            .append("java -jar ").append("clustalo.jar")
                            .append(" --paramDetails <parameter-name>")
                            .append(params.getIds().size() > 0 ? "\n\nFor example:\njava -jar clustalo.jar --paramDetails " + params.getIds().get(0) : "");
            ...
            // Details of selected parameter
                ...
                if (parameter != null) {
                    ...
                    ClientUtilsclustalo.marshallToXML(parameter, debugLevel, "parameters");
                } else {
            ...
            // Submit new job
                ...
                String jobid = null;
                if (sequence != null){
                    jobid = submitJob(cli, ClientUtilsclustalo.loadData(sequence), null);
                } else if (asequence != null && bsequence != null) {
                    jobid = submitJob(cli, ClientUtilsclustalo.loadData(asequence),
                                             ClientUtilsclustalo.loadData(bsequence));
                }
                ...
                if (jobid != null) {
                    ...
                    if (cli.hasOption("async")) {
                        ...
                        printDebugMessage("run", String.format("You can check status of your job with:\n" +
                                                 "java -jar clustalo.jar --status --jobid %s\n", jobid), 11);

                        if (outputLevel > 0)
                            System.out.println("To check status: java -jar clustalo.jar --status --jobid " + jobid);
                    ...
                    // Synchronous execution
                        ...
                        if (ClientUtilsclustalo.getStatusInIntervals(this, jobid, pollFreq, outputLevel, debugLevel)) {
            ...
            // Check already submitted job
                ...
                if (cli.hasOption("resultTypes")) {
                    ...
                    ClientUtilsclustalo.marshallToXML(getResultTypesForJobId(jobid), debugLevel, "types");
                    if (outputLevel > 0)
                        System.out.println("To get results:\n  java -jar clustalo.jar --polljob --jobid " + jobid
                                           + "\n  java -jar clustalo.jar --polljob --outformat <type> --jobid " + jobid);
                }
    ...
    public void downloadResults(CommandLine cli, String jobid) throws IOException {
        ...
        if (cli.hasOption("outformat")) {
            resultTypes = ClientUtilsclustalo.processUserOutputFormats(cli.getOptionValue("outformat"),
    ...
    private String submitJob(CommandLine cli, String inputSeq, String inputSeq2)
        ...
        if (cli.hasOption("guidetreeout") == false)
           form.putSingle("guidetreeout", "true");

        if (cli.hasOption("dismatout") == false)
           form.putSingle("dismatout", "true");

        if (cli.hasOption("dealign") == false)
           form.putSingle("dealign", "false");

        if (cli.hasOption("mbed") == false)
           form.putSingle("mbed", "true");

        if (cli.hasOption("mbediteration") == false)
           form.putSingle("mbediteration", "true");

        if (cli.hasOption("iterations") == false)
           form.putSingle("iterations", "0");

        if (cli.hasOption("gtiterations") == false)
           form.putSingle("gtiterations", "-1");

        if (cli.hasOption("hmmiterations") == false)
           form.putSingle("hmmiterations", "-1");
