/* $Id$
 * ======================================================================
 * JDispatcher EMBOSS sixpack (SOAP) web service Java client using Axis 1.x.
 * ----------------------------------------------------------------------
 * Tested with:
...
import uk.ac.ebi.webservices.axis1.stubs.emboss.sixpack.*;

/** <p>JDispatcher EMBOSS sixpack (SOAP) web service Java client using Apache 
 * Axis 1.x.</p>
 * 
 * <p>See:</p>
 * <ul>
 * <li><a href="http://www.ebi.ac.uk/Tools/webservices/services/psa/emboss_sixpack_soap">http://www.ebi.ac.uk/Tools/webservices/services/psa/emboss_sixpack_soap</a></li>
 ...
 * </ul>
 */
public class EmbossSixpackClient extends uk.ac.ebi.webservices.AbstractWsToolClient {
	...
	private static final String usageMsg = "EMBOSS sixpack\n"
		+ "================\n"
		+ "\n"
		...
		+ "      --firstorf     :      : enable to count the beginning of a sequence as\n" 
		+ "	                             a possible ORF\n"
		+ "      --nofirstorf   :      : disable  counting the beginning of a sequence\n"
		+ "                              as a possible ORF\n"
		+ "      --lastorf      :      : enable to count the end of a sequence as a \n"
		+ "                              possible ORF\n"
		+ "      --nolastorf    :      : disable counting the end of a sequence as a \n"
		+ "                              possible ORF\n"
		+ "      --reverse      :      : enable to reverse\n"
		+ "      --noreverse    :      : disable reversing\n"
		+ "      --orfminsize   : int  : minimum size of Open Reading Frames (ORFs) to \n" 
		+ "                              display in the translations\n";	
	
	/** Default constructor.
	 */
	public EmbossSixpackClient() {
	...
	public InputParameters loadParams(CommandLine line) throws IOException {
		...
		if (line.hasOption("firstorf")) params.setFirstorf(new Boolean(true));
		else if (line.hasOption("nofirstorf")) params.setFirstorf(new Boolean(false));
		if (line.hasOption("lastorf")) params.setLastorf(new Boolean(true));
		else if (line.hasOption("nolastorf")) params.setLastorf(new Boolean(false));
		...
		if (line.hasOption("orfminsize")) params.setOrfminsize(line.getOptionValue("orfminsize"));
	...
	public static void main(String[] args) {
		...
		options.addOption("firstorf", false, "Enable to count the beginning of a sequence as a possible ORF");
		options.addOption("nofirstorf", false, "Disable counting the beginning of a sequence as a possible ORF");
		options.addOption("lastorf", false, "Enable to count the end of a sequence as a possible ORF");
		options.addOption("nolastorf", false, "Disable counting the end of a sequence as a possible ORF");
		options.addOption("reverse", false, "Enable to reverse");
		options.addOption("noreverse", false, "Disable reversing");
		options.addOption("orfminsize", true, "Minimum size of Open Reading Frames (ORFs) to display in the translation");
		...
		try {
			...
			// Submit a job
				...
				if (cli.hasOption("async")) {
					...
					System.err.println("To get status: java -jar EmbossSixpack_Axis1.jar --status --jobid " + jobid);
				} else {
