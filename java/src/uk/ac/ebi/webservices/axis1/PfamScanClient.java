 ...
 * 
 * ======================================================================
 * JDispatcher PfamScan (SOAP) web service Java client using Axis 1.x.
 * ----------------------------------------------------------------------
 * Tested with:
...
import uk.ac.ebi.webservices.axis1.stubs.pfamscan.*;

/**
 * <p>
 * JDispatcher PfamScan (SOAP) web service Java client using Apache Axis
 * 1.x.
 * </p>
 * 
 * <p>
 ...
 * <ul>
 * <li><a
 * href="http://www.ebi.ac.uk/Tools/webservices/services/sss/pfamscan_soap"
 * >http://www.ebi.ac.uk/Tools/webservices/services/sss/pfamscan_soap</a></li>
 * <li><a
 ...
 * </ul>
 */
public class PfamScanClient extends uk.ac.ebi.webservices.AbstractWsToolClient {
	/** Service proxy */
	private JDispatcherService_PortType srvProxy = null;
	/** Client version/revision */
	private String revision = "$Revision$";
	/** Tool specific usage message */
	private static final String usageMsg = "PfamScan\n"
			+ "==============\n"
			+ "\n"
			+ "Search a protein sequence against Pfam.\n" 
			+ "\n"
			+ "For information see:\n"
			+ "- http://www.ebi.ac.uk/Tools/webservices/services/pfa/pfamscan_soap\n"
			+ "\n"
			+ "[Required]\n"
			+ "\n"
	...
	public InputParameters loadParams(CommandLine line) throws IOException {
		...
		if(line.hasOption("D")) {
			params.setDatabase(line.getOptionValue("D"));
		}
		if(line.hasOption("E")) {
			params.setEvalue(Double.valueOf(line.getOptionValue("E")));
		}
		if(line.hasOption("F")) {
			params.setFormat(line.getOptionValue("F"));
		}
		if(line.hasOption("asp")) {
			params.setAsp(true);
		}
		else if(line.hasOption("noasp")) {
			params.setAsp(false);
		}
	...
	public String submitJobFromCli(CommandLine cli, String inputSeq)
		...
		if (cli.hasOption("async")) {
			...
			System.err
					.println("To get status: java -jar PfamScan_Axis1.jar --status --jobid "
							+ jobid);
		}
	...
	public static void main(String[] args) {
		...
		options.addOption("D", "database", true, "Pfam database");
		options.addOption("E", "evalue", true, "E-value threshold");
		options.addOption("F", "format", true, "Result format");
		options.addOption("asp", "asp", false, "Enable active site prediction");
		options.addOption("noasp", "noasp", false, "Disable active site prediction");
		...
		PfamScanClient client = new PfamScanClient();
		try {
