/* $Id$
 * ======================================================================
 * JDispatcher MAFFT (SOAP) web service Java client using Axis 1.x.
 * ----------------------------------------------------------------------
 * Tested with:
...
import uk.ac.ebi.webservices.axis1.stubs.mafft.*;

/** <p>JDispatcher MAFFT (SOAP) web service Java client using Apache 
 * Axis 1.x.</p>
 * 
 * <p>See:</p>
 * <ul>
 ...
 * </ul>
 */
public class MafftClient extends uk.ac.ebi.webservices.AbstractWsToolClient {
	...
	private static final String usageMsg = "MAFFT\n"
		+ "=====\n"
		+ "\n"
	...
	public InputParameters loadParams(CommandLine line) throws IOException {
		...
		if (line.hasOption("nbtree")) params.setNbtree(new Integer(line.getOptionValue("nbtree")));
		if (line.hasOption("maxiterate")) params.setMaxiterate(new Integer(line.getOptionValue("maxiterate")));
		if (line.hasOption("ffts")) params.setFfts(line.getOptionValue("ffts"));
	...
	public static void main(String[] args) {
		...
		options.addOption("nbtree", true, "Tree rebuilding number");
		options.addOption("maxiterate", true, "Maximum number of iterations");
		options.addOption("ffts", true, "Perform FFTS");
		...
		MafftClient client = new MafftClient();
		try {
			...
			// Submit a job
				...
				if (cli.hasOption("async")) {
					...
					System.err.println("To get status: java -jar Mafft_Axis1.jar --status --jobid " + jobid);
				} else {
