/* $Id$
 * ======================================================================
 * JDispatcher Readseq (SOAP) web service Java client using Axis 1.x.
 * ----------------------------------------------------------------------
 * Tested with:
...
import uk.ac.ebi.webservices.axis1.stubs.readseq.*;

/** <p>JDispatcher Readseq (SOAP) web service Java client using Apache Axis 1.x.</p>
 * 
 * <p>See:</p>
 * <ul>
 * <li><a href="http://www.ebi.ac.uk/Tools/webservices/services/sfc/readseq_soap">http://www.ebi.ac.uk/Tools/webservices/services/sfc/readseq_soap</a></li>
 ...
 * </ul>
 */
public class ReadseqClient extends uk.ac.ebi.webservices.AbstractWsToolClient {
	...
	private static final String usageMsg = "Readseq\n"
		+ "================\n"
		+ "\n"
		+ "Manipulate and reformat sequences.\n"
		+ "\n"    
		+ "[Required]\n"
		+ "\n"
		...
		+ "[Optional]\n"
		+ "\n"
		+ "      --inputformat  : int  : input sequence format, see --paramDetail inputformat\n"
		+ "      --outputformat : int  : output sequence format, see --paramDetail outputformat\n"
		+ "      --outputcase   : str  : output sequence character case, see --paramDetail outputcase\n"
		+ "      --reverse      :      : reverse-complement input nucleotide sequence\n"
		+ "      --noreverse    :      : don't reverse-complement input nucleotide sequence\n"
		+ "      --degap        : str  : symbols to remove from sequence\n"
		+ "      --transymbol   : str  : pairs of symbols to replace, see --paramDetail transymbol\n"
		+ "      --feature      : str  : list of features to select, see --paramDetail feature\n"
		+ "      --fthandle     : str  : action to perform on selected features, see --paramDetail fthandle\n"
		+ "      --subrange     : str  : region of sequence to perform feature processing\n"
		+ "";

	
	/** Default constructor.
	 */
	public ReadseqClient() {
	...
	public InputParameters loadParams(CommandLine line) throws IOException {
		...
		if (line.hasOption("inputformat")) params.setInputformat(Integer.valueOf(line.getOptionValue("inputformat")));
		if (line.hasOption("outputformat")) params.setOutputformat(Integer.valueOf(line.getOptionValue("outputformat")));
		if (line.hasOption("outputcase")) params.setOutputcase(line.getOptionValue("outputcase"));
		if (line.hasOption("degap")) params.setDegap(line.getOptionValue("degap"));
		if (line.hasOption("transymbol")) params.setTransymbol(line.getOptionValue("transymbol"));
		if (line.hasOption("fthandle")) params.setFthandle(line.getOptionValue("fthandle"));
		if (line.hasOption("subrange")) params.setSubrange(line.getOptionValue("subrange"));
		
		if (line.hasOption("reverse")) params.setReverse(Boolean.TRUE);
	...
	public static void main(String[] args) {
		...
		options.addOption("inputformat", true, "Input sequence format");
		options.addOption("outputformat", true, "Output sequence format");
		options.addOption("outputcase", true, "Output sequence character case");
		options.addOption("reverse", false, "Reverse-complement sequence");
		options.addOption("noreverse", false, "Don't reverse-complement sequence (default)");
		options.addOption("degap", true, "Symbols to remove from sequence");
		options.addOption("transymbol", true, "pairs of symbols to replace");
		options.addOption("feature", true, "list of features to select");
		options.addOption("fthandle", true, "action to perform on selected features");
		options.addOption("subrange", true, "region of sequence to perform feature processing");
		...
		try {
			...
			// Submit a job
				...
				if (cli.hasOption("async")) {
					...
					System.err.println("To get status: java -jar Readseq_Axis1.jar --status --jobid " + jobid);
				} else {
