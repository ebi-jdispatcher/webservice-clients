 ...
 * 
 * ======================================================================
 * JDispatcher PROSITE Scan (SOAP) web service Java client using Axis 1.x.
 * ----------------------------------------------------------------------
 * Tested with:
...
import uk.ac.ebi.webservices.axis1.stubs.ps_scan.*;

/** <p>JDispatcher PROSITE Scan (SOAP) web service Java client using Apache 
 * Axis 1.x.</p>
 * 
 * <p>See:</p>
 * <ul>
 * <li><a href="http://www.ebi.ac.uk/Tools/webservices/services/msa/ps_scan_soap">http://www.ebi.ac.uk/Tools/webservices/services/msa/ps_scan_soap</a></li>
 ...
 * </ul>
 */
public class PsScanClient extends uk.ac.ebi.webservices.AbstractWsToolClient {
	...
	private static final String usageMsg = "PROSITE Scan\n"
		+ "=============\n"
		+ "\n"
		...
		+ "                                --paramDetail format.\n"
		+ "  --database           : str  : database to scan, see \n"
		+ "                                --paramDetail database.\n"
		+ "  --scanControl        : str  : scan Patterns and/or Profiles, see \n"
		+ "                                --paramDetail scanControl.\n"
		...
		+ "  --profileThreshold   : int  : cut-off level for profiles, see \n"
		+ "                                --paramDetail profileThreshold.\n"
	...
	public InputParameters loadParams(CommandLine line) throws IOException {
		...
		if (line.hasOption("scanControl")) params.setScanControl(line.getOptionValue("scanControl"));
		if (line.hasOption("commonMatch")) {
			params.setCommonMatch(new Boolean(true));
		}
		else if (line.hasOption("noCommonMatch")) {
			params.setCommonMatch(new Boolean(false));
		}
		if (line.hasOption("overlappingMatch")) {
			params.setOverlappingMatch(new Boolean(true));
		}
		else if (line.hasOption("noOverlappingMatch")) {
			params.setOverlappingMatch(new Boolean(false));
		}
		if (line.hasOption("profileThreshold")) params.setProfileThreshold(new Integer(line.getOptionValue("profileThreshold")));
	...
	public String submitJobFromCli(CommandLine cli, String inputSeq)
		...
		if (cli.hasOption("async")) {
			...
			System.err
					.println("To get status: java -jar PsScan_Axis1.jar --status --jobid "
							+ jobid);
		}
	...
	public static void main(String[] args) {
		...
		options.addOption("database", true, "Database to scan");
		options.addOption("scanControl", true, "Scan Patterns and/or Profiles");
		options.addOption("commonMatch", false, "Enable common matches reporting");
		options.addOption("noCommonMatch", false, "Disable common matches reporting");
		options.addOption("overlappingMatch", false, "Enable overlapping matches reporting");
		options.addOption("noOverlappingMatch", false, "Disable overlapping matches reporting");
		options.addOption("profileThreshold", true, " Cut-off level for profiles");
		...
		PsScanClient client = new PsScanClient();
		try {
