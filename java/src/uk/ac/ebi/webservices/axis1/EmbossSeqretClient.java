 ...
 * 
 * ======================================================================
 * JDispatcher EMBOSS seqret (SOAP) web service Java client using Axis 1.x.
 * ----------------------------------------------------------------------
 * Tested with:
...
import uk.ac.ebi.webservices.axis1.stubs.emboss.seqret.*;

/** <p>JDispatcher EMBOSS seqret (SOAP) web service Java client using Apache Axis 1.x.</p>
 * 
 * <p>See:</p>
 * <ul>
 * <li><a href="http://www.ebi.ac.uk/Tools/webservices/services/sfc/emboss_seqret_soap">http://www.ebi.ac.uk/Tools/webservices/services/sfc/emboss_seqret_soap</a></li>
 ...
 * </ul>
 */
public class EmbossSeqretClient extends uk.ac.ebi.webservices.AbstractWsToolClient {
	...
	public InputParameters loadParams(CommandLine line) throws IOException {
		...
		if (line.hasOption("inputformat")) params.setInputformat(line.getOptionValue("inputformat"));
		...
		if (line.hasOption("feature")) params.setFeature(Boolean.TRUE);
		else if (line.hasOption("nofeature")) params.setFeature(Boolean.FALSE);
		if (line.hasOption("firstonly")) params.setFirstonly(Boolean.TRUE);
		else if (line.hasOption("nofirstonly")) params.setFirstonly(Boolean.FALSE);
	...
	public void submitJobFromCli(CommandLine cli, String inputSeq)
		...
		if (cli.hasOption("async")) {
			...
			System.err
					.println("To get status: java -jar EmbossSeqret_Axis1.jar --status --jobid "
							+ jobid);
		} else {
	...
	public static void main(String[] args) {
		...
		options.addOption("inputformat", true, "Format for input data");
		options.addOption("outputformat", true, "Format for output data");
		options.addOption("feature", false, "Use feature information");
		options.addOption("nofeature", false, "Do not use feature information (default)");
		options.addOption("firstonly", false, "Only first input sequence");
		options.addOption("nofirstonly", false, "All input sequences (default)");
		options.addOption("reverse", false, "Reverse-complement nucleotide input sequence");
		options.addOption("noreverse", false, "Original nucleotide input sequence (default)");
		options.addOption("outputcase", true, "Character case for output sequence data");
		...
		EmbossSeqretClient client = new EmbossSeqretClient();
		try {
