...
import uk.ac.ebi.webservices.axis1.stubs.emboss.newcpgreport.*;

/** <p>JDispatcher EMBOSS newcpgreport (SOAP) web service Java client using Apache 
 * Axis 1.x.</p>
 * 
 * <p>See:</p>
 * <ul>
 * <li><a href="http://www.ebi.ac.uk/Tools/webservices/services/psa/emboss_newcpgreport_soap">http://www.ebi.ac.uk/Tools/webservices/services/psa/emboss_newcpgreport_soap</a></li>
 ...
 * </ul>
 */
public class EmbossNewcpgreportClient extends uk.ac.ebi.webservices.AbstractWsToolClient {
	...
	private static final String usageMsg = "EMBOSS newcpgreport\n"
			+ "===================\n"
			+ "\n"
			...
			+ "\n"
			+ "[Optional]\n"
			+ "      --minlen        : int  : minimum length of a CpG island.\n"
			+ "      --minoe         : real : minumun observed/expected ratio.\n"
			+ "      --minpc         : real : minimum CpG percentage.\n"
			+ "      --window        : int  : windows size for CpG calculation.\n"
			+ "      --multifasta    :      : treat input as a set of fasta formatted sequences\n";

	/** Default constructor.
	 */
	public EmbossNewcpgreportClient() {
	...
	public InputParameters loadParams(CommandLine line) throws IOException {
		...
		if (line.hasOption("minlen")) params.setMinlen(new Integer(line.getOptionValue("minlen")));
		if (line.hasOption("minoe")) params.setMinoe(new Double(line.getOptionValue("minoe")));
		if (line.hasOption("minpc")) params.setMinpc(new Double(line.getOptionValue("minpc")));
		if (line.hasOption("window")) params.setWindow(new Integer(line.getOptionValue("window")));
	...
	public String submitJobFromCli(CommandLine cli, String inputSeq)
		...
		if (cli.hasOption("async")) {
			...
			System.err
					.println("To get status: java -jar EmbossNewcpgreport_Axis1.jar --status --jobid "
							+ jobid);
		}
	...
	public static void main(String[] args) {
		...
		options.addOption("minlen", true, "minimum length of a CpG island.");	
		options.addOption("minoe", true, "minumun observed/expected ratio.");	
		options.addOption("minpc", true, "minimum CpG percentage.");	
		options.addOption("window", true, "windows size for CpG calculation.");	
		...
		EmbossNewcpgreportClient client = new EmbossNewcpgreportClient();
		try {
