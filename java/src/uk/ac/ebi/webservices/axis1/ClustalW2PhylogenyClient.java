 ...
 * 
 * ======================================================================
 * JDispatcher ClustalW2 Phylogeny (SOAP) web service Java client using 
 * Axis 1.x.
 * ----------------------------------------------------------------------
 * Tested with:
...
import uk.ac.ebi.webservices.axis1.stubs.clustalw2phylogeny.*;

/** <p>JDispatcher ClustalW2 Phylogeny (SOAP) web service Java client using 
 * Apache Axis 1.x.</p>
 * 
 * <p>See:</p>
 * <ul>
 * <li><a href="http://www.ebi.ac.uk/Tools/webservices/services/phylogeny/clustalw2_phylogeny_soap">http://www.ebi.ac.uk/Tools/webservices/services/phylogeny/clustalw2_phylogeny_soap</a></li>
 ...
 * </ul>
 */
public class ClustalW2PhylogenyClient extends uk.ac.ebi.webservices.AbstractWsToolClient {
	...
	private static final String usageMsg = "ClustalW2 Phylogeny\n"
		+ "===================\n"
		+ "\n"
	...
	public InputParameters loadParams(CommandLine line) throws IOException {
		...
		if (line.hasOption("C")) params.setClustering(line.getOptionValue("C"));
		if (line.hasOption("K")) {
			params.setKimura(new Boolean(true));
		}
		else {
			params.setKimura(new Boolean(false));
		}
		if (line.hasOption("I")) {
			params.setTossgaps(new Boolean(true));
		}
		else {
			params.setTossgaps(new Boolean(false));
		}
		if (line.hasOption("P")) {
			params.setPim(new Boolean(true));
		}
		else {
			params.setPim(new Boolean(false));
		}
	...
	public void submitJobFromCli(CommandLine cli, String inputData)
		...
		if (cli.hasOption("async")) {
			...
			System.err
					.println("To get status: java -jar ClustalW2Phylogeny_Axis1.jar --status --jobid "
							+ jobid);
		} else {
	...
	public static void main(String[] args) {
		...
		options.addOption("t", "tree", true, "Tree format");
		options.addOption("K", "kimura", false, "Kimura distance correction");
		options.addOption("I", "tossgaps", false, "Exclude gaps");
		options.addOption("C", "clustering", true, "Clustering method for tree");
		options.addOption("P", "pim", false, "Percent identitity matrix");
