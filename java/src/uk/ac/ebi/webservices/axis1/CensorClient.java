 ...
 * 
 * ======================================================================
 * JDispatcher CENSOR (SOAP) web service Java client using Axis 1.x.
 * ----------------------------------------------------------------------
 * Tested with:
...
import uk.ac.ebi.webservices.axis1.stubs.censor.*;

/**
 * <p>
 * JDispatcher CENSOR (SOAP) web service Java client using Apache Axis 1.x.
 * </p>
 * 
 * <p>
 * See:
 ...
 * <ul>
 * <li><a
 * href="http://www.ebi.ac.uk/Tools/webservices/services/so/censor_soap"
 * >http://www.ebi.ac.uk/Tools/webservices/services/so/censor_soap</a></li>
 * <li><a
 ...
 * </ul>
 */
public class CensorClient extends uk.ac.ebi.webservices.AbstractWsToolClient {
	...
	public InputParameters loadParams(CommandLine line) throws IOException {
		...
		if(line.hasOption("l")) params.setDatabase(line.getOptionValue("l"));
		if (line.hasOption("b")) params.setMode(line.getOptionValue("b"));
		if (line.hasOption("t")) {
			params.setTranslate(new Boolean(true));
		}
		else if (line.hasOption("notranslate")) {
			params.setTranslate(new Boolean(false));
		}
		if (line.hasOption("d")) {
			params.setIdentity(new Boolean(true));
		}
		else if (line.hasOption("noidentity")) {
			params.setIdentity(new Boolean(false));
		}
		if (line.hasOption("s")) {
			params.setShowsimple(new Boolean(true));
		}
		else if (line.hasOption("noshowsimple")) {
			params.setShowsimple(new Boolean(false));
		}
		if (line.hasOption("m")) {
			params.setMaskpseudo(new Boolean(true));
		}
		else if (line.hasOption("nomaskpseudo")) {
			params.setMaskpseudo(new Boolean(false));
		}
	...
	public String submitJobFromCli(CommandLine cli, String inputSeq)
		...
		if (cli.hasOption("async")) {
			...
			System.err
					.println("To get status: java -jar Censor_Axis1.jar --status --jobid "
							+ jobid);
		}
	...
	public static void main(String[] args) {
		...
		options.addOption("l", "database", true, "name of library/database file");
		options.addOption("b", "mode", true, "search sensitivity mode");
		options.addOption("t", "translate", false, "translate sequence");
		options.addOption("notranslate", "notranslate", false, "do not translate sequence");
		options.addOption("m", "maskpseudo", false, "mask pseuso genes");
		options.addOption("nomaskpseudo", "nomaskpseudo", false, "do not mask pseuso genes");
		options.addOption("d", "identity", false, "identity search");
		options.addOption("noidentity", "noidentity", false, "not identity search");
		options.addOption("s", "showsimple", false, "show simple repeats");
		options.addOption("noshowsimple", "noshowsimple", false, "do not show simple repeats");
		...
		CensorClient client = new CensorClient();
		try {
