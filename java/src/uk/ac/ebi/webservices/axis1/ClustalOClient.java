 ...
 * 
 * ======================================================================
 * JDispatcher ClustalO (SOAP) web service Java client using Axis 1.x.
 * ----------------------------------------------------------------------
 * Tested with:
...
import uk.ac.ebi.webservices.axis1.stubs.clustalo.*;

/** <p>JDispatcher ClustalO (SOAP) web service Java client using Apache 
 * Axis 1.x.</p>
 * 
 * <p>See:</p>
 * <ul>
 * <li><a href="http://www.ebi.ac.uk/Tools/webservices/services/msa/clustalo_soap">http://www.ebi.ac.uk/Tools/webservices/services/msa/clustalo_soap</a></li>
 ...
 * </ul>
 */
public class ClustalOClient extends uk.ac.ebi.webservices.AbstractWsToolClient {
	...
	private static final String usageMsg = "Clustal Omega\n"
		+ "=============\n"
		+ "\n"
	...
	public InputParameters loadParams(CommandLine line) throws IOException {
		...
		else if (line.hasOption("noguidetreeout")) {
			params.setGuidetreeout(new Boolean(false));
		}
		if (line.hasOption("dismatout")) {
			params.setDismatout(new Boolean(true));
		}
		else if (line.hasOption("nodismatout")) {
			params.setDismatout(new Boolean(false));
		}
		if (line.hasOption("dealign")) {
			params.setDealign(new Boolean(true));
		}
		else if (line.hasOption("nodealign")) {
			params.setDealign(new Boolean(false));
		}
		if (line.hasOption("mbed")) {
			params.setMbed(new Boolean(true));
		}
		else if (line.hasOption("nombed")) {
			params.setMbed(new Boolean(false));
		}
		if (line.hasOption("mbediteration")) {
			params.setMbediteration(new Boolean(true));
		}
		else if (line.hasOption("nombediteration")) {
			params.setMbediteration(new Boolean(false));
		}
		if (line.hasOption("iterations")) params.setIterations(new Integer(line.getOptionValue("iterations")));
		if (line.hasOption("gtiterations")) params.setGtiterations(new Integer(line.getOptionValue("gtiterations")));
		if (line.hasOption("hmmiterations")) params.setHmmiterations(new Integer(line.getOptionValue("hmmiterations")));
		if (line.hasOption("outfmt")) params.setOutfmt(line.getOptionValue("outfmt"));
	...
	public String submitJobFromCli(CommandLine cli, String inputData)
		...
		if (cli.hasOption("async")) {
			...
			System.err
					.println("To get status: java -jar ClustalO_Axis1.jar --status --jobid "
							+ jobid);
		} else {
	...
	public static void main(String[] args) {
		...
		options.addOption("guidetreeout", false, "Output guide tree");
		options.addOption("noguidetreeout", false, "Output guide tree");
		options.addOption("dismatout", false, "Output distance matrix");
		options.addOption("nodismatout", false, "Output distance matrix");
		options.addOption("dealign", false, "Dealign input sequences");
		options.addOption("nodealign", false, "Dealign input sequences");
		options.addOption("mbed", false, "Mbed-like clustering guide-tree");
		options.addOption("nombed", false, "Mbed-like clustering guide-tree");
		options.addOption("mbediteration", false, "Mbed-like clustering iteration");
		options.addOption("nombediteration", false, "Mbed-like clustering iteration");
		options.addOption("iterations", true, "Number of iterations");
		options.addOption("gtiterations", true, "Maximum guild tree iterations");
		options.addOption("hmmiterations", true, "Maximum HMM iterations");
		options.addOption("outfmt", true, "Output alignment format");
		options.addOption("stype", true, "Input sequence type");
