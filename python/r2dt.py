...
#
# R2DT (REST) web service Python client using xmltramp2.
#
...
def print_usage():
    print("""\
EMBL-EBI R2DT Python Client:

RNA analysis with R2DT.

[Required (for job submission)]
  --email               E-mail address.
  --sequence            One or more RNA sequences are required. There is currently a
                        sequence input limit of 10000 sequences and 1MB of data.
...
[General]
  ...
  --baseUrl             Base URL. Defaults to:
                        https://www.ebi.ac.uk/Tools/services/rest/r2dt

Synchronous job:
  The results/errors are returned as soon as the job is finished.
  Usage: python r2dt.py --email <your@email.com> [options...] <SeqFile|SeqID(s)>
  Returns: results as an attachment

Asynchronous job:
  Use this if you want to retrieve the results at a later time. The results
  are stored for up to 24 hours.
  Usage: python r2dt.py --asyncjob --email <your@email.com> [options...] <SeqFile|SeqID(s)>
  Returns: jobid

Check status of Asynchronous job:
  Usage: python r2dt.py --status --jobid <jobId>

Retrieve job data:
  Use the jobid to query for the status of the job. If the job is finished,
  it also returns the results/errors.
  Usage: python r2dt.py --polljob --jobid <jobId> [--outfile string]
