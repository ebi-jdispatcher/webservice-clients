...
#
# EMBOSS cons (REST) web service Python client using xmltramp2.
#
...
baseUrl = u'https://www.ebi.ac.uk/Tools/services/rest/emboss_cons'
...
parser.add_option('--plurality', type=str, help=('Cut-off for the number of positive matches below which there is no'
                  'consensus.'))
parser.add_option('--setcase', type=str, help=('The threshold for the positive matches above which the consensus is in'
                  'upper-case and below which the consensus is in lower-case.'))
parser.add_option('--name', type=str, help=('Name of the consensus sequence.'))
parser.add_option('--identity', type=int, help=('The required number of identities at a site for it to give a consensus'
                  'at that position.'))
...
def print_usage():
    print("""\
EMBL-EBI EMBOSS cons Python Client:

Consensus sequences with EMBOSS Cons.
...
[Optional]
  --plurality           Cut-off for the number of positive matches below which there
                        is no consensus.
  --setcase             The threshold for the positive matches above which the
                        consensus is in upper-case and below which the consensus is
                        in lower-case.
  --name                Name of the consensus sequence.
  --identity            The required number of identities at a site for it to give a
...
[General]
  ...
  --baseUrl             Base URL. Defaults to:
                        https://www.ebi.ac.uk/Tools/services/rest/emboss_cons

Synchronous job:
  The results/errors are returned as soon as the job is finished.
  Usage: python emboss_cons.py --email <your@email.com> [options...] <SeqFile|SeqID(s)>
  Returns: results as an attachment

Asynchronous job:
  Use this if you want to retrieve the results at a later time. The results
  are stored for up to 24 hours.
  Usage: python emboss_cons.py --asyncjob --email <your@email.com> [options...] <SeqFile|SeqID(s)>
  Returns: jobid

Check status of Asynchronous job:
  Usage: python emboss_cons.py --status --jobid <jobId>

Retrieve job data:
  Use the jobid to query for the status of the job. If the job is finished,
  it also returns the results/errors.
  Usage: python emboss_cons.py --polljob --jobid <jobId> [--outfile string]
...
# Submit job
    ...
    if options.plurality:
        params['plurality'] = options.plurality
    

    if options.setcase:
        params['setcase'] = options.setcase
