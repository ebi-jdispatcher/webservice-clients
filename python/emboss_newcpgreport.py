...
#
# EMBOSS newcpgreport (REST) web service Python client using xmltramp2.
#
...
baseUrl = u'https://www.ebi.ac.uk/Tools/services/rest/emboss_newcpgreport'
...
Synchronous job:
  ...
  Usage: python emboss_newcpgreport.py --email <your@email.com> [options...] <SeqFile|SeqID(s)>
  Returns: results as an attachment

Asynchronous job:
  Use this if you want to retrieve the results at a later time. The results
  are stored for up to 24 hours.
  Usage: python emboss_newcpgreport.py --asyncjob --email <your@email.com> [options...] <SeqFile|SeqID(s)>
  Returns: jobid

Check status of Asynchronous job:
  Usage: python emboss_newcpgreport.py --status --jobid <jobId>

Retrieve job data:
  Use the jobid to query for the status of the job. If the job is finished,
  it also returns the results/errors.
  Usage: python emboss_newcpgreport.py --polljob --jobid <jobId> [--outfile string]
...
# Submit job
    ...
    if options.minlen:
        params['minlen'] = options.minlen
    

    if options.minoe:
        params['minoe'] = options.minoe
    

    if options.minpc:
        params['minpc'] = options.minpc
