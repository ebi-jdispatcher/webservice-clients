...
baseUrl = u'http://www.ebi.ac.uk/Tools/services/rest/mapmi'
...
description = u'''\
    MapMi, a tool for mapping and analysis of miRNA sequences. It is designed to locate miRNA precursor sequences in existing genomic sequences (e.g Ensembl and Ensembl Metazoa), using potential mature miRNA sequences as input. After searching the genome for the provided mature sequences, these hits are extended and classified taking into account major structural properties of known miRNA precursors.\
'''
epilog = u'''For further information about the MAPMI web service, see
http://www.ebi.ac.uk/tools/webservices/services/rna/mapmi_rest.'''
version = u'ee8df29'
...
parser.add_option('--bowtie', help='Maximum allowed matches per candidate genome (Bowtie-m). 0 for disable.')
parser.add_option('--excludecan', help='Exclude candidates in loop. Binary toggle to exclude mature.')
parser.add_option('--maxloop', help='Maximum loop overlap. Maximum allowed mature basepairs overlapping loop.')
parser.add_option('--sequence', help='The query sequence(s) can be entered directly into this form. The sequence can be in GCG, FASTA, EMBL, GenBank, PIR, NBRF or PHYLIP format. A partially formatted sequence is not accepted. Adding a return to the end of the sequence may help certain applications understand the input. Note that directly using data from word processors may yield unpredictable results as hidden/control characters may be present. There is a limit of 1MB for the sequence entry.')
