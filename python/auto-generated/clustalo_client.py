...
description = u'''\
    Clustal Omega is a multiple sequence alignment program. It produces biologically meaningful multiple sequence alignments of divergent sequences. Evolutionary relationships can be seen via viewing Cladograms or Phylograms.\
'''
...
parser.add_option('--guidetreeout', help='Output guide tree.')
parser.add_option('--dismatout', help='Output distance matrix. This is only calculated if the mBed-like clustering guide tree is set to false.')
parser.add_option('--dealign', help='Remove any existing alignment (gaps) from input sequences.')
parser.add_option('--mbed', help='This option uses a sample of the input sequences and then represents all sequences as vectors to these sequences, enabling much more rapid generation of the guide tree, especially when the number of sequences is large.')
parser.add_option('--mbediteration', help='Use mBed-like clustering during subsequent iterations.')
parser.add_option('--iterations', help='Number of (combined guide-tree/HMM) iterations.')
parser.add_option('--gtiterations', help='Having set the number of combined iterations, this parameter can be changed to limit the number of guide tree iterations within the combined iterations.')
parser.add_option('--hmmiterations', help='Having set the number of combined iterations, this parameter can be changed to limit the number of HMM iterations within the combined iterations.')
...
parser.add_option('--sequence', help='Three or more sequences to be aligned can be entered directly into this box. Sequences can be in GCG, FASTA, EMBL (Nucleotide only), GenBank, PIR, NBRF, PHYLIP or UniProtKB/Swiss-Prot (Protein only) format. Partially formatted sequences are not accepted. Adding a return to the end of the sequence may help certain applications understand the input. Note that directly using data from word processors may yield unpredictable results as hidden/control characters may be present.\
\	\	\	There is currently a sequence input limit of 4000 sequences and 4MB of data.\
\	\	\	')
# General options
