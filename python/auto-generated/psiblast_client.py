...
baseUrl = u'http://www.ebi.ac.uk/Tools/services/rest/psiblast'
...
'''
epilog = u'''For further information about the PSI BLAST web service, see
...
parser.add_option('--expthr', help='Limits the number of scores and alignments reported based on the expectation value. This is the maximum number of times the match is expected to occur by chance.')
parser.add_option('--psithr', help='Expectation value threshold for automatic selection of matched sequences for inclusion in PSSM at each iteration.')
...
parser.add_option('--sequence', help='The query sequence can be entered directly into this form. The sequence can be in GCG, FASTA, PIR, NBRF, PHYLIP or UniProtKB/Swiss-Prot format. A partially formatted sequence is not accepted. Adding a return to the end of the sequence may help certain applications understand the input. Note that directly using data from word processors may yield unpredictable results as hidden/control characters may be present.')
...
parser.add_option('--selectedHits', help='List of identifiers of the hits from the previous iteration to use to construct the search PSSM for this iteration.')
...
parser.add_option('--patfile', help='Pattern file for PHI-BLAST functionality. This file needs to be in the style of a prosite entry file, with at least an ID line, PA line and optional HI line.')
# General options
