...
baseUrl = u'http://www.ebi.ac.uk/Tools/services/rest/psisearch'
...
parser.add_option('--gapopen', help='Penalty taken away from the score when a gap is created in sequence. Increasing the gap opening penalty will decrease the number of gaps in the final alignment.')
parser.add_option('--gapext', help='Penalty taken away from the score for each base or residue in the gap. Increasing the gap extension penalty favours short gaps in the final alignment, conversly, decreasing the gap extension penalty favours long gaps in the final alignment.')
parser.add_option('--expthr', help='Limits the number of scores and alignments reported based on the expectation value. This value is the maximum number of times the match is expected to occur by chance.')
parser.add_option('--mask', help='Turn on/off the sequence masking for HOEs in PSSM constructions. This option allows you to mask sequence characters beyond the alignment region when constructing the PSSM, reducing over-extension errors.')
parser.add_option('--psithr', help='Expectation value threshold for automatic selection of matched sequences for inclusion in the PSSM at each iteration.')
parser.add_option('--scores', help='Maximum number of alignment score summaries reported in the result output.')
parser.add_option('--alignments', help='Maximum number of alignments reported in the result output.')
parser.add_option('--hsps', help='Turn on/off the display of all significant alignments between query and database sequence.')
...
parser.add_option('--hist', help='Turn on/off the histogram in the PSI-Search result. The histogram gives a qualitative view of how well the statistical theory fits the similarity scores calculated by the program.')
parser.add_option('--annotfeats', help='Turn on/off annotation features. \
...
\	\	\	\	domain regions.\
\	\	\	')
parser.add_option('--sequence', help='The query sequence can be entered directly into this form.\
\	\	\	\	The sequence can be in GCG, FASTA, PIR, NBRF, PHYLIP or UniProtKB/Swiss-Prot format.\
\	\	\	\	A partially formatted sequence is not accepted. Adding a return to the end of the sequence may help certain applications understand the input.\
\	\	\	\	Note that directly using data from word processors may yield unpredictable results as hidden/control characters may be present.\
            ')
parser.add_option('--database', help='The databases to run the sequence similarity search against. Multiple databases can be selected at the same time.')
parser.add_option('--previousjobid', help='The job identifier for the previous PSI-Search iteration.')
parser.add_option('--selectedHits', help='List of identifiers from the hits of the previous iteration to use to construct the search PSSM for this iteration.')
parser.add_option('--bdrfile', help='Boundary file containing boundary information for pre-selected sequences.Used for hardmask to clean HOEs. ')
parser.add_option('--cpfile', help='Checkpoint file from the previous iteration. Must be in ASN.1 Binary Format.')
# General options
