...
#
# EMBOSS dottup (REST) web service Python client using xmltramp2.
#
...
def print_usage():
    print("""\
EMBL-EBI EMBOSS dottup Python Client:
...
Synchronous job:
  ...
  Usage: python emboss_dottup.py --email <your@email.com> [options...] <SeqFile|SeqID(s)>
  Returns: results as an attachment

Asynchronous job:
  Use this if you want to retrieve the results at a later time. The results
  are stored for up to 24 hours.
  Usage: python emboss_dottup.py --asyncjob --email <your@email.com> [options...] <SeqFile|SeqID(s)>
  Returns: jobid

Check status of Asynchronous job:
  Usage: python emboss_dottup.py --status --jobid <jobId>

Retrieve job data:
  Use the jobid to query for the status of the job. If the job is finished,
  it also returns the results/errors.
  Usage: python emboss_dottup.py --polljob --jobid <jobId> [--outfile string]
...
# Submit job
    ...
    if not options.boxit:
        params['boxit'] = 'true'
    if options.boxit:
        params['boxit'] = options.boxit
