...
#
# MapMi (REST) web service Python client using xmltramp2.
#
...
Synchronous job:
  ...
  Usage: python mapmi.py --email <your@email.com> [options...] <SequenceFile>
  Returns: results as an attachment

Asynchronous job:
  Use this if you want to retrieve the results at a later time. The results
  are stored for up to 24 hours.
  Usage: python mapmi.py --async --email <your@email.com> [options...] <SequenceFile>
  Returns: jobid

  Use the jobid to query for the status of the job. If the job is finished,
  it also returns the results/errors.
  Usage: python mapmi.py --polljob --jobid <jobId> [--outfile string]
