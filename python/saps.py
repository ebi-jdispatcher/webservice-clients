...
#
# SAPS (REST) web service Python client using xmltramp2.
#
...
Synchronous job:
  ...
  Usage: python saps.py --email <your@email.com> [options...] <SeqFile|SeqID(s)>
  Returns: results as an attachment

Asynchronous job:
  Use this if you want to retrieve the results at a later time. The results
  are stored for up to 24 hours.
  Usage: python saps.py --asyncjob --email <your@email.com> [options...] <SeqFile|SeqID(s)>
  Returns: jobid

Check status of Asynchronous job:
  Usage: python saps.py --status --jobid <jobId>

Retrieve job data:
  Use the jobid to query for the status of the job. If the job is finished,
  it also returns the results/errors.
  Usage: python saps.py --polljob --jobid <jobId> [--outfile string]
...
# Submit job
    ...
    if not options.outputtype:
        params['outputtype'] = 'default'
    if options.outputtype:
        params['outputtype'] = options.outputtype
    

    if not options.species:
        params['species'] = 'swp23s'
    if options.species:
        params['species'] = options.species
    

    if not options.positiveresidues:
        params['positiveresidues'] = 'lys-arg'
    if options.positiveresidues:
        params['positiveresidues'] = options.positiveresidues
