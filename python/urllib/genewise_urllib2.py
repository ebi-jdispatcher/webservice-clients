#!/usr/bin/env python
# $Id: genewise_urllib2.py 2809 2017-02-14 16:10:25Z afoix $
# ======================================================================
#
...
#
# ======================================================================
# GENEWISE (REST) Python client using urllib2 and
...
baseUrl = 'http://www.ebi.ac.uk/Tools/services/rest/genewise'
# Load libraries
...
description = """GeneWise compares a protein sequence to a genomic DNA sequence, allowing for introns and frameshifting errors."""
epilog = """For further information about the GENEWISE (REST) web service, see http://www.ebi.ac.uk/Tools/webservices/services/psa/genewise_rest."""
version = "$Id: genewise_urllib2.py 2809 2017-02-10 16:10:25Z afoix $"
# Process command-line options
parser = OptionParser(usage=usage, description=description, epilog=epilog, version=version)
# Tool specific options
parser.add_option('--para', help='Show parameters in output alignmment')
parser.add_option('--nopara', help='Don t show parameters in output alignmment')
parser.add_option('--pretty', help='Show pretty ASCII alignment viewing')
parser.add_option('--nopretty', help='Don t show pretty ASCII alignment viewing')
parser.add_option('--genes', help='Show gene structure')
parser.add_option('--nogenes', help='Don t show gene structure')
parser.add_option('--trans', help='Show protein translation, breaking at frameshifts')
parser.add_option('--notrans', help='Don t show protein translation, breaking at frameshifts')
parser.add_option('--cdna', help='Show cDNA')
parser.add_option('--nocdna', help='Don t show cDNA')
parser.add_option('--embl', help='EMBL feature table format with CDS key')
parser.add_option('--noembl', help='Don t output EMBL feature table format with CDS key')
parser.add_option('--ace', help='Show Ace file gene structure')
parser.add_option('--noace', help='Don t show Ace file gene structure')
parser.add_option('--gff', help='Show Gene Feature Format file')
parser.add_option('--nogff', help='Don t show Gene Feature Format file')
parser.add_option('--diana', help='Show EMBL FT format suitable for diana')
parser.add_option('--nodiana', help='Don t show EMBL FT format suitable for diana')
parser.add_option('--init', help='Model in local/global mode')
parser.add_option('--splice', help='Splice site')
parser.add_option('--random', help='Random (Null) Model')
parser.add_option('--alg', help='Algorithm')
...
# Submit job
    ...
    if len(args) > 0:
        if os.access(args[0], os.R_OK):  # Read file into content
            ...
            params['bsequence'] = readFile(args[0])
        else:  # Argument is a sequence id
            params['asequence'] = args[0]
            params['bsequence'] = args[0]
    elif options.asequence and options.asequence:  # Specified via option
        if os.access(options.asequence, os.R_OK) and os.access(options.bsequence, os.R_OK):  # Read file into content
    ...
    if options.para:
        ...
        params['nopara'] = options.nopara
    elif options.pretty:
        params['pretty'] = options.pretty
    elif options.nopretty:
        params['nopretty'] = options.nopretty
    elif options.genes:
        params['genes'] = options.genes
    elif options.nogenes:
        params['nogenes'] = options.nogenes
    elif options.trans:
        params['trans'] = options.trans
    elif options.notrans:
        params['notrans'] = options.notrans
    elif options.cdna:
        params['cdna'] = options.cdna
    elif options.nocdna:
        params['nocdna'] = options.nocdna
    elif options.embl:
        params['embl'] = options.embl
    elif options.noembl:
        params['noembl'] = options.noembl
    elif options.ace:
        params['ace'] = options.ace
    elif options.noace:
        params['noace'] = options.noace
    elif options.gff:
        params['gff'] = options.gff
    elif options.nogff:
        params['nogff'] = options.nogff
    elif options.diana:
        params['diana'] = options.diana
    elif options.nodiana:
        params['nodiana'] = options.nodiana
    elif options.init:
