...
#
# ======================================================================
# PSI-Search (REST) Python 2 client using urllib2 and
...
description = """Rapid sequence database search programs utilizing the PSI-Search algorithm. For more information
on PSI-Search refer to http://www.ebi.ac.uk/Tools/sss/psisearch"""
epilog = """For further information about the PSI-Search (REST) web service, see
http://www.ebi.ac.uk/Tools/webservices/services/sss/psisearch_rest."""
version = "$Id: psisearch_urllib2.py 2106 2012-05-01 17:00:40Z afoix $"
...
parser.add_option('-E', '--expthr', help='E-value threshold')
...
parser.add_option('--mask', type='int', help='PSSM region masking')
parser.add_option('--hsps', help='enable multiple alignments per-hit,\
                                  see --paramDetail hsps')
parser.add_option('--nohsps', help='disable multiple alignments per-hit,\
                                    see --paramDetail hsps')
parser.add_option('--annotfeats', help='enable annotation features,\
                                        see --paramDetail annotfeats')
parser.add_option('--noannotfeats', help='disable annotation features,\
                                          see --paramDetail annotfeats')
...
parser.add_option('--multifasta', help='Multiple fasta input')

# Compatability options, old command-line
parser.add_option('--opengap', help='Open gap penalty')
parser.add_option('--extendgap', help='Gap extension penalty')
parser.add_option('--exp', help='E-value threshold')
parser.add_option('--expmulti', help='E-value for hit selection')
parser.add_option('--preselseq', help='Selected hits for PSSM')
parser.add_option('--checkpoint', help='PSI-BLAST checkpoint')
# General options
...
# Submit job
    ...
    if options.nohsps:
        params['nohsps'] = options.nohsps
    if options.annotfeats:
        params['annotfeats'] = options.annotfeats
    if options.noannotfeats:
        params['noannotfeats'] = options.noannotfeats
    ...
    if options.multifasta:
        params['multifasta'] = options.multifasta
    if options.opengap:
    ...
    if options.expmulti:
        params['expmulti'] = options.expmulti
    if options.preselseq:
        params['preselseq'] = options.preselseq
