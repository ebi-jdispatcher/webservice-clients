...
#
# EMBOSS dotpath (REST) web service Python client using xmltramp2.
#
...
Asynchronous job:
  ...
  Usage: python emboss_dotpath.py --asyncjob --email <your@email.com> [options...] <SeqFile|SeqID(s)>
  Returns: jobid
...
# Submit job
    ...
    if not options.overlaps:
        params['overlaps'] = 'false'
    if options.overlaps:
        params['overlaps'] = options.overlaps
