...
baseUrl = u'http://www.ebi.ac.uk/Tools/services/rest/emboss_sixpack'
...
'''
epilog = u'''For further information about the EMBOSS sixpack web service, see
...
parser.add_option('--codontable', help='Which Genetic Code table to use. These are kept synchronised with those maintained at the NCBI\'s Taxonomy Browser. ')
parser.add_option('--firstorf', help='Count the beginning of a sequence as a possible ORF, even if it\'s inferior to the minimal ORF size.')
parser.add_option('--lastorf', help='Count the end of a sequence as a possible ORF, even if it\'s not finishing with a STOP, or inferior to the minimal ORF size.')
parser.add_option('--reverse', help='Choose this option if you wish to reverse and compliment your sequence.')
parser.add_option('--orfminsize', help='Minimum size of Open Reading Frames (ORFs) to display in the translations.')
parser.add_option('--sequence', help='A DNA sequence can be entered or pasted into this box. Ideally use a known sequence format such as fasta or EMBL, text pasted from word processors may contain meta-characters that cause problems.\
\	\	\	')
# General options
...
# Submit job
    ...
    if options.firstorf:
        ...
        params['firstorf'] = False
    if options.lastorf:
        params['lastorf'] = True
    else:
        params['lastorf'] = False
    if options.reverse:
        params['reverse'] = True
    else:
