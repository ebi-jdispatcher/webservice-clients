...
baseUrl = u'http://www.ebi.ac.uk/Tools/services/rest/emboss_seqret'
...
'''
epilog = u'''For further information about the EMBOSS SEQRET web service, see
...
parser.add_option('--feature', help='Use feature information')
parser.add_option('--firstonly', help='Read one sequence and stop')
parser.add_option('--reverse', help='Reverse-complement of input DNA sequences')
parser.add_option('--outputcase', help='Change alphabet case for output sequences.')
parser.add_option('--seqrange', help='Specify a range or section of the input sequence to use in the search. Example: Specifying \'34-89\' in an input sequence of total length 100, will tell EMBOSS seqret to only use residues 34 to 89, inclusive.')
parser.add_option('--sequence', help='One or more sequences to be translated can be entered directly into this form. Sequences can be in GCG, FASTA, EMBL (Nucleotide only), GenBank, PIR, NBRF, PHYLIP or UniProtKB/Swiss-Prot (Protein only) format. Partially formatted sequences are not accepted. Adding a return to the end of the sequence may help certain applications understand the input. Note that directly using data from word processors may yield unpredictable results as hidden/control characters may be present. There is a limit of 2MB.')
# General options
