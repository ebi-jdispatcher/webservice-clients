...
#
# ======================================================================
# Simple_Phylogeny (REST) web service Python-2 client using urllib2 and
...
baseUrl = 'http://www.ebi.ac.uk/Tools/services/rest/simple_phylogeny'

print >> sys.stderr, """
=============================================================================
Simple phylogeny:
Commonly used phylogenetic tree generation methods provided by the ClustalW2
=============================================================================
"""
...
description = """Simple Phylogeny (REST) web service Python client"""
epilog = """For further information see
http://www.ebi.ac.uk/Tools/webservices/services/phylogeny/simple_phylogeny_rest"""
version = "$Id: simple_phylogeny_lwp.py 2791 2014-05-29 15:31:39Z hpm"
# Process command-line options
parser = OptionParser(usage=usage, description=description, epilog=epilog, version=version)
# Tool specific options
# parser.add_option('--appl', help='signature methods to use, see --paramDetail appl')
parser.add_option('--tree', help='Enable output guide tree')
parser.add_option('--kimura', help='Kimura distance correction')
parser.add_option('--tossgaps', help='Dealign input sequences')
parser.add_option('--clustering', help='Clustering method')
parser.add_option('--pim', help='% identity matrix.')
parser.add_option('--sequence', help='Input sequences/alignment')
# Compatability options, old command-line
parser.add_option('--outputtree', help='Phylogenetic tree format')
# General options
...
# Submit job
    ...
    if options.kimura is not None:
        params['kimura'] = options.kimura
    if options.tossgaps is not None:
        params['tossgaps'] = options.tossgaps
    if options.pim is not None:
