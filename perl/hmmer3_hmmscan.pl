...
HMMER 3 hmmscan (REST) web service Perl client using L<LWP>.
...
sub load_params {
    ...
    if ($params{'cut_ga'}) {
        $tool_params{'cut_ga'} = 1;
    }
    else {
        $tool_params{'cut_ga'} = 0;
    }
    if ($params{'nobias'}) {
...
[Optional]
  --incE                : str  : Significance E-values[Model]
  --incdomE             : str  : Significance E-values[Hit]
  --E                   : str  : Report E-values[Model]
  --domE                : str  : Report E-values[Hit]
  --incT                : str  : Significance bit scores[Sequence]
  --incdomT             : str  : Significance bit scores[Hit]
  --T                   : str  : Report bit scores[Sequence]
  --domT                : str  : Report bit scores[Hit]
  --cut_ga              : bool : Use the gathering threshold.
  --nobias              : bool : Filters
  --hmmdbparam          : str  : hmmdbparam
  --alignView           : bool : Output alignment in result
  --database            : str  : HMM Database
