...
Clustal Omega (REST) web service Perl client using L<LWP>.
...
[Optional]
  ...
  --dismatout           : bool : Output distance matrix. This is only calculated if the mBed-like
                                 clustering guide tree is set to false.
  --dealign             : bool : Remove any existing alignment (gaps) from input sequences.
  --mbed                : bool : This option uses a sample of the input sequences and then represents
                                 ...
                                 is large.
  --mbediteration       : bool : Use mBed-like clustering during subsequent iterations.
  --iterations          : int  : Number of (combined guide-tree/HMM) iterations.
  --gtiterations        : int  : Having set the number of combined iterations, this parameter can be
                                 changed to limit the number of guide tree iterations within the
                                 combined iterations.
  --hmmiterations       : int  : Having set the number of combined iterations, this parameter can be
                                 changed to limit the number of HMM iterations within the combined
                                 iterations.
  --outfmt              : str  : Format for generated multiple sequence alignment.
  --order               : str  : The order in which the sequences appear in the final alignment
