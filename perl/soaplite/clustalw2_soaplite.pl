...
my $WSDL = 'http://www.ebi.ac.uk/Tools/services/soap/clustalw2?wsdl';
...
GetOptions(
    ...
    'alignment|l=s' => \$tool_params{'alignment'}, # Pairwise alignment method
    'type=s'        => \$tool_params{'type'},      # Sequence type: DNA or protein
    'ktup|k=i'      => \$tool_params{'ktup'},      # Word size
    'window|w=i'    => \$tool_params{'window'},    # Window size
    'score|s=s'     => \$tool_params{'score'},     # Pairwise score representation
    'topdiags|d=i'  => \$tool_params{'topdiags'},  # Number of best diags.
    'pairgap|p=i'   => \$tool_params{'pairgap'},   # Gap penalty (pairwise)
    'pwmatrix=s'    => \$tool_params{'pwmatrix'},  # Protein matrix (pairwise)
    'pwdnamatrix=s' => \$tool_params{'pwdnamatrix'}, # DNA matrix (pairwise)
    'pwgapopen=i'   => \$tool_params{'pwgapopen'}, # Gap open penalty (pairwise)
    'pwgapext=f'    => \$tool_params{'pwgapext'},  # Gap extension penalty (pairwise)
    'matrix|m=s'    => \$tool_params{'matrix'},    # Protein scoring matrix (multi)
    'dnamatrix=s'   => \$tool_params{'dnamatrix'}, # DNA scoring matrix (multi)
    'gapopen|g=i'   => \$tool_params{'gapopen'},   # Gap creation penalty (multi)
    'noendgaps'     => \$tool_params{'noendgaps'}, # Terminal gaps scored
    'gapext|x=f'    => \$tool_params{'gapext'},    # Gap extension penalty (multi)
