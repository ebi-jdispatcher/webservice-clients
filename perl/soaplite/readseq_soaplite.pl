...
Readseq (SOAP) web service Perl client using L<SOAP::Lite>.
...
=item *
L<http://www.ebi.ac.uk/Tools/webservices/services/sfc/readseq_soap>
...
my $WSDL = 'http://www.ebi.ac.uk/Tools/services/soap/readseq?wsdl';
...
sub load_params {
	...
	# -feature: list of feature types to select.
	if ( $params{'feature'} ) {
		my (@optList) = split /[ ,]/, $params{'feature'};
		for ( my $i = 0 ; $i < scalar(@optList) ; $i++ ) {
			$tool_params{'feature'}[$i] =
			  SOAP::Data->type( 'string' => $optList[$i] )->name('string');
		}
	}
