...
=item *
L<http://www.ebi.ac.uk/Tools/webservices/services/msa/prank_soap>
...
my $WSDL = 'http://wwwdev.ebi.ac.uk/Tools/services/soap/prank?wsdl';
...
sub load_data {
	...
	if ( defined( $ARGV[0] ) ) {
		...
		# Identifier (DB:ID) or raw sequence
		else {
			$retSeq = $ARGV[0];
		}
	}
...
=head2 load_param_file()

Load data from a file specified as a parameter argument.

  my $data = load_param_file($params{'param_name'});
 
=cut

sub load_param_file {
	print_debug_message( 'load_param_file', 'Begin', 2 );
	my $retData = undef;
	my $paramData = shift;
	if($paramData) {
		...
		# Otherwise assume raw data, or identifier understood by service
		else {
			$retData = $paramData;
		}
	}
	print_debug_message( 'load_param_file', 'End', 2 );
	return $retData;
}
...
Further information:
  ...
  http://www.ebi.ac.uk/Tools/webservices/services/msa/prank_soap
