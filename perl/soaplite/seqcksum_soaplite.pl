...
seqcksum_soaplite.pl

=head1 DESCRIPTION

SeqCksum (SOAP) web service Perl client using L<SOAP::Lite>.
...
=item *
L<http://www.ebi.ac.uk/Tools/webservices/services/so/seqcksum_soap>
...
Copyright 2013-2013 EMBL - European Bioinformatics Institute
...
my $WSDL = 'http://www.ebi.ac.uk/Tools/services/soap/seqcksum?wsdl';
...
my %params = (
	'debugLevel' => 0,
	'maxJobs'    => 1,
	#'cksmethod' => 'crc spcrc altcrc crcdigest md shaa shab shac shad shae'
);
...
GetOptions(
	...
	'stype=s'      => \$tool_params{'stype'},    # Molecule type (DNA or Protein)
	'sequence=s'   => \$params{'sequence'},      # Query sequence file or DB:ID
	'cksmethod=s'  => \$params{'cksmethod'},     # Checksum method(s)
	'length'       => \$params{'length'},        # Show length
	'nolength'     => \$params{'nolength'},      # Do not show length
...
sub load_params {
	...
	# Checksum method(s) to use
	if(defined($params{'cksmethod'})) {
		my (@ckList) = split /[ ,]/, $params{'cksmethod'};
		for ( my $i = 0 ; $i < scalar(@ckList) ; $i++ ) {
			$tool_params{'cksmethod'}[$i] =
		  		SOAP::Data->type( 'string' => $ckList[$i] )->name('string');
		}
	}
