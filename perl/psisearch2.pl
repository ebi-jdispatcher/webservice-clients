...
PSI-Search 2 (REST) web service Perl client using L<LWP>.
...
GetOptions(
    ...
    'psiprogram=s'    => \$params{'psiprogram'},     # Select the source doamin database to query.
    ...
    'maxiterations=i' => \$params{'maxiterations'},  # Run iteration(s) until reaching convergence or the maximum number of iterations.
    'mask=s'          => \$params{'mask'},           # Select method to mask sequence regions for HOEs in PSSM constructions. This option allows you to mask sequence characters beyond the alignment region when constructing the PSSM, reducing over-extension errors.
    'domainannot=s'   => \$params{'domainannot'},    # Produce domain annotation for query, subject or both.
    'domaindb=s'      => \$params{'domaindb'},       # Select the source domain database to query.
    'domaindiag'      => \$params{'domaindiag'},     # Display domain diagrams in SVG
    'seqbdr=s'        => \$params{'seqbdr'},         # Options to determine sequence boundaries for PSSM generation (alignment, domain or combination).
    'qvalue=i'        => \$params{'qvalue'},         # Domain Q-value threshold (larger than) to include domain boundaries for PSSM generation.
...
sub usage {
    print STDERR <<EOF
EMBL-EBI PSI-Search 2 Perl Client:
