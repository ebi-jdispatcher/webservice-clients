...
EMBOSS cons (REST) web service Perl client using L<LWP>.
...
my $baseUrl = 'https://www.ebi.ac.uk/Tools/services/rest/emboss_cons';
...
GetOptions(
    ...
    'plurality=s'     => \$params{'plurality'},      # Cut-off for the number of positive matches below which there is no consensus.
    'setcase=s'       => \$params{'setcase'},        # The threshold for the positive matches above which the consensus is in upper-case and below which the consensus is in lower-case.
    'name=s'          => \$params{'name'},           # Name of the consensus sequence.
    'identity=i'      => \$params{'identity'},       # The required number of identities at a site for it to give a consensus at that position.
...
sub usage {
    print STDERR <<EOF
EMBL-EBI EMBOSS cons Perl Client:
