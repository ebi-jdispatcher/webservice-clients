...
[Optional]
  --program             : str  : The BLAST program to be used for the Sequence Similarity Search.
  --task                : str  : Task option (only selectable for blastn)
  ...
  --exp                 : str  : Limits the number of scores and alignments reported based on the
                                 expectation value. This is the maximum number of times the match is
                                 expected to occur by chance.
  --dropoff             : int  : The amount a score can drop before gapped extension of word hits is
                                 halted
  ...
  --gapalign            : bool : This is a true/false setting that tells the program the perform
  ...
  --compstats           : str  : Use composition-based statistics.
  --align               : int  : Formating for the alignments
  --transltable         : int  : Query Genetic code to use in translation
  --stype               : str  : Indicates if the sequence is protein or DNA/RNA.
