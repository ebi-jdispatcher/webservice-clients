...
PredComp (REST) web service Perl client using L<LWP>.
...
=item *
L<http://wwwdev.ebi.ac.uk/Tools/webservices/services/pfa/predcomp_rest>

=item *
L<http://wwwdev.ebi.ac.uk/Tools/webservices/tutorials/perl>
...
my $baseUrl = 'http://wwwdev.ebi.ac.uk/Tools/services/rest/predcomp';
#my $baseUrl = 'http://ashdev-2.ebi.ac.uk:21110/Tools/services/rest/predcomp';
...
#	'uptable=s'     => \$params{'uptable'},      # Query input
...
# Submit a job
	...
	if (( defined( $params{'uptable'} ) && $params{'uptable'} =~ m/^\@/ )
		|| ( defined( $ARGV[0] ) && $ARGV[0] =~ m/^\@/ ) )
	{
		my $list_filename = $params{'uptable'} || $ARGV[0];
		$list_filename =~ s/^\@//;
	...
	for ( my $jobNum = ( scalar(@$jobid_list) - 1 ) ; $jobNum > -1 ; $jobNum-- )
	{
		  ...
		  )
		{
			if ( $job_status eq 'ERROR' || $job_status eq 'FAILED' ) {
				print STDERR
"Warning: job $jobid failed for input $job_number: $seq_id\n";
			}
			&get_results( $jobid, $seq_id );
...
Further information:
  ...
  http://wwwdev.ebi.ac.uk/Tools/webservices/services/pfa/predcomp_rest
  http://wwwdev.ebi.ac.uk/Tools/webservices/tutorials/perl

Support/Feedback:

  http://wwwdev.ebi.ac.uk/support/
EOF
}
