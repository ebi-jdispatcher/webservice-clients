...
RAxML_EPA (REST) web service Perl client using L<LWP>.
...
=item *
L<http://www.ebi.ac.uk/Tools/webservices/services/phylogeny/raxml_epa_rest>
...
my $baseUrl = 'http://www.ebi.ac.uk/Tools/services/rest/raxml_epa';
...
GetOptions(
    ...
    'alignment=s'   => \$params{'alignment'},      # Input alignment file
    'newick=s'      => \$params{'newick'},         # Input Newick tree file
    'algorithm=s'   => \$tool_params{'algorithm'}, # Algorithm to use
...
if (
    !(
        ...
        (defined($ARGV[0]) && defined($ARGV[1]))
            || (defined($params{'alignment'})
            && defined($params{'newick'}))
    )
) {
...
Load data from file or option specified on the command-line.
...
sub load_data {
    ...
    if ($params{'alignment'}) {                                       # Via --alignment
        if (-f $params{'alignment'} || $params{'alignment'} eq '-') { # File
    ...
    if ($params{'newick'}) {                                    # Via --alignment
        if (-f $params{'newick'} || $params{'newick'} eq '-') { # File
...
[Required]
  ...
  --newick           : file : Input Newick tree file
  --email            : str  : e-mail address

[Optional]

  --algorithm        : str  : Algorithm to use [v|y]
...
Synchronous job:
  ...
  Usage: $scriptName --email <your\@email> [options...] --alignment alignFile --newick newickFile
...
Further information:
  ...
  http://www.ebi.ac.uk/Tools/webservices/services/phylogeny/raxml_epa_rest
