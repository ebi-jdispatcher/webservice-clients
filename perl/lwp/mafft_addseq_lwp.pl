...
=item *
L<http://www.ebi.ac.uk/Tools/webservices/services/msa/mafft_addseq_rest>
...
my $baseUrl = 'http://www.ebi.ac.uk/Tools/services/rest/mafft_addseq';
...
GetOptions(
    ...
    'fastaalign|f=s' => \$params{'fastaalign'},    # Input FASTA alignment
...
sub load_params {
	...
	if ( $params{'fastaalign'} ) {                   # Via --fastaalign
		if ( -f $params{'fastaalign'} ) {    # File
			$tool_params{'fastaalign'} = &read_file( $params{'fastaalign'} );
		}
		else {
			die 'Error: no FASTA Alignment report specified'
		}
	}
...
[Required]
  ...
  -f, --fastaalign   : file : Input FASTA alignment
...
Further information:
  ...
  http://www.ebi.ac.uk/Tools/webservices/services/msa/mafft_addseq_rest
