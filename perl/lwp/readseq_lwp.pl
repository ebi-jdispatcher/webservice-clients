...
Readseq (REST) web service Perl client using L<LWP>.
...
=item *
L<http://www.ebi.ac.uk/Tools/webservices/services/sfc/readseq_rest>
...
my $baseUrl = 'http://www.ebi.ac.uk/Tools/services/rest/readseq';
...
sub load_params {
	...
	if ( $params{'feature'} ) {
		...
		$tool_params{'feature'} = \@optList;
	}

	# -reverse: reverse-complement sequence.
	if ( $params{'reverse'} ) {
