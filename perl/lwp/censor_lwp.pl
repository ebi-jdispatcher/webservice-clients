...
CENSOR (REST) web service Perl client using L<LWP>.
...
=item *
L<http://www.ebi.ac.uk/Tools/webservices/services/so/censor_rest>
...
my $baseUrl = 'http://www.ebi.ac.uk/Tools/services/rest/censor';
...
GetOptions(
	...
	'database|l=s'   => \$tool_params{'database'},  # Database(s) to search
	'mode|b=s'       => \$tool_params{'mode'},      # Sensitivity mode
	'translate|t'    => \$params{'translate'},      # Translate input
	'notranslate'    => \$params{'notranslate'},    # Don't translate input
	'maskpseudo|m'   => \$params{'maskpseudo'},     # Mask pseudogenes
	'nomaskpseudo|m' => \$params{'nomaskpseudo'},   # Don't mask pseudogenes
	'identity|d'	 => \$params{'identity'},       # Identical matches only
	'noidentity'	 => \$params{'noidentity'},     # All matches
	'showsimple|s'   => \$params{'showsimple'},     # Show simple repeats
	'noshowsimple'   => \$params{'noshowsimple'},   # Don't show simple repeats
...
sub load_params {
	...
	# translate
	if ( $params{'translate'} ) {
		$tool_params{'translate'} = 'true';
	}
	elsif ( $params{'notranslate'} ) {
		$tool_params{'translate'} = 'true';
	}
	# maskpseudo
	if ( $params{'maskpseudo'} ) {
		$tool_params{'maskpseudo'} = 'true';
	}
	elsif ( $params{'nomaskpseudo'} ) {
		$tool_params{'maskpseudo'} = 'true';
	}
	# identity
	if ( $params{'identity'} ) {
		$tool_params{'identity'} = 'true';
	}
	elsif ( $params{'noidentity'} ) {
		$tool_params{'identity'} = 'true';
	}
	# showsimple
	if ( $params{'showsimple'} ) {
		$tool_params{'showsimple'} = 'true';
	}
	elsif ( $params{'noshowsimple'} ) {
		$tool_params{'showsimple'} = 'true';
	}
...
Further information:
  ...
  http://www.ebi.ac.uk/Tools/webservices/services/so/censor_rest
