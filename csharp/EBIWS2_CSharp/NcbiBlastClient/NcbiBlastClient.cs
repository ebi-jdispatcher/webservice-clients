...
";
            ...
            for (int i = 0; i < args.Length; i++)
            {
                ...
                switch (args[i])
                {
                    ...
                    default:
                        ...
                        if (args[i] == "-" || File.Exists(args[i]))
                        { // Option is a file
                            InParams.sequence = ReadTextFile(args[i]);
                        }
                        else
                        { // Entry ID or raw sequence
        ...
        public override void GetResults(string jobId, string outformat, string outFileBase)
        {
            ...
            { // Specified data type
                ...
                {
                    WriteFile(OutFile + "." + selResultType.fileSuffix, res);
                }
            }
            else
            { // Data types available
                ...
                foreach (wsResultType resultType in resultTypes)
                {
                    ...
                    {
                        WriteFile(OutFile + "." + resultType.fileSuffix, res);
                    }
                }
            }
