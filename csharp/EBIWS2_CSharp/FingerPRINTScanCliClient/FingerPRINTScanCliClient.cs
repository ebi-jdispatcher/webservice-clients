 ...
 * 
 * ======================================================================
 * jDispatcher SOAP command-line client for FingerPRINTScan (SOAP).
 * ====================================================================== */
using System;
using System.IO;
using EbiWS.FingerPRINTScanWs;

namespace EbiWS
{
	class FingerPRINTScanCliClient : EbiWS.FingerPRINTScanClient
	{
		/// <summary>Tool specific usage</summary>
		private string usageMsg = @"FingerPRINTScan
=============
...
";
		...
		public static int Main(string[] args)
		{
			...
			FingerPRINTScanCliClient wsApp = new FingerPRINTScanCliClient();
		...
		private void ParseCommand(string[] args)
		{
			...
			for (int i = 0; i < args.Length; i++)
			{
				...
				switch (args[i])
				{
					...
					case "/matrix":
						goto case "--matrix";
					case "--evalue": // Expectation value threshold
						InParams.evalue = Convert.ToDouble(args[++i]);
						InParams.evalueSpecified = true;
						break;
					case "/evalue":
						goto case "--evalue";
					case "--distdev": // Distance deviation
						InParams.distdev = Convert.ToInt32(args[++i]);
						InParams.distdevSpecified = true;
						break;
					case "/distdev":
						goto case "--distdev";	
					case "--resultviews": // Output result views
						char[] sepList = { ' ', ',' };
						InParams.resultviews = args[++i].Split(sepList);
						break;
					case "/resultviews":
						goto case "--resultviews";
