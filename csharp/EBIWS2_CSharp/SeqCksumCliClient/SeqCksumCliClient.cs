 ...
 * 
 * ======================================================================
 * jDispatcher SOAP command-line client for SeqCksum
 * ====================================================================== */
using System;
using System.IO;
using EbiWS.SeqCksumWs;

namespace EbiWS
{
	class SeqCksumCliClient : EbiWS.SeqCksumClient
	{
		/// <summary>Tool specific usage</summary>
		private string usageMsg = @"SeqCksum
========
...
";
		...
		public static int Main(string[] args)
		{
			...
			SeqCksumCliClient wsApp = new SeqCksumCliClient();
		...
		private void ParseCommand(string[] args)
		{
			...
			for (int i = 0; i < args.Length; i++)
			{
				...
				switch (args[i])
				{
					...
					case "--cksmethod": // Checksum methods.
						char[] sepList = { ' ', ',' };
						InParams.cksmethod = args[++i].Split(sepList);
						Action = "submit";
						break;
					case "/cksmethod":
						goto case "--cksmethod";
					...
					case "/stype":
						goto case "--stype";
					case "--length": // Report sequence length
						InParams.length = true;
						InParams.lengthSpecified = true;
						break;
					case "/length":
						goto case "--length";
					case "--nolength": // Do not report sequence length
						InParams.length = false;
						InParams.lengthSpecified = true;
						break;
					case "/nolength":
						goto case "--nolength";
